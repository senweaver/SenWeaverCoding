# Design System — Skeumorphism

> Category: Morphism & Effects
> Tactile real-world textures: material surfaces, beveled edges, and realistic depth cues.

## 1. Visual Theme & Atmosphere

Skeuomorphism mimics physical objects — stitched leather, brushed metal, beveled buttons, and realistic shadows. Controls look pressable; surfaces imply material. Depth and texture sell the metaphor.

**Key characteristics**
- Realistic material surfaces and bevels
- Pronounced, believable shadows and highlights
- Pressable, physical-feeling controls
- Texture and gradient imply real materials
...
**Do**
- Use beveled edges and realistic depth.
- Imply material with gradients/texture.
- Make controls look physically pressable.
- Bind `tokens.css` into `:root` and reference every value via `var(--*)`.

**Don't**
- Don't flatten everything into pure flat color.
- Don't remove highlights that sell the bevel.
- Don't mix incompatible material metaphors.
- Don't invent off-palette colors or redefine the token contract.

## 10. Agent Prompt Guide

- **Hero:** `--bg #f7eee6` canvas; `.eyebrow` uppercase `--muted` → headline Georgia 600 `--text-4xl` (`--leading-tight`, tracking `-0.025em`) `--fg` → `.lead` `--fg-2` `--text-xl` → `.btn-primary` (`--accent #b46a46`) + `.btn-secondary`.
- **Card:** `--surface` fill, `--border` structure, `--radius-lg 34px`, `--elev-raised` when raised; title `--fg` 600, body `--fg-2`.
- **Iteration rules:** (1) realistic material surfaces and bevels; (2) accent `#b46a46` drives interaction; (3) keep type in Georgia/Inter; (4) honor the spacing + radius scale exactly.
