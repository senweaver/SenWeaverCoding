# Design System — Expressive

> Category: Bold & Expressive
> Artful, opinionated design: bold type pairings, asymmetric layout, and personality-forward color.

## 1. Visual Theme & Atmosphere

Expressive design has a point of view — confident type pairings, asymmetric compositions, and personality-rich color. It breaks the grid intentionally to feel crafted and human.

**Key characteristics**
- Bold, contrasting type pairings
- Asymmetric, intentional compositions
- Personality-forward color choices
- Crafted, hand-considered details
...
| Role | Token | Value |
|------|-------|-------|
| Background | `--bg` | `#fbf8ff` |
| Surface | `--surface` | `#ffffff` |
| Surface warm | `--surface-warm` | `#efe8ff` |
| Foreground | `--fg` | `#1f1730` |
| Foreground 2 | `--fg-2` | `#4d425f` |
| Muted | `--muted` | `#776b88` |
| Border | `--border` | `#ded4ee` |
...
- **Buttons** — `.btn-primary`: `--accent` fill, `--accent-on` text, radius `--radius-md 16px`, padding `--space-2`/`--space-5`; hover → `--accent-hover`. `.btn-secondary`: `--surface` fill, 1px `--border`, `--fg` text. Focus-visible: `--focus-ring 0 0 0 4px rgba(124, 58, 237, 0.24)`.
...
| Raised | `--elev-raised` | `0 22px 58px rgba(87, 49, 139, 0.13)` |
...
**Do**
- Pair contrasting type with intent.
- Break the grid intentionally.
- Inject personality through color.
- Bind `tokens.css` into `:root` and reference every value via `var(--*)`.

**Don't**
- Don't default to safe symmetric layouts.
- Don't use generic type pairings.
- Don't sand off all personality.
- Don't invent off-palette colors or redefine the token contract.

## 10. Agent Prompt Guide

- **Hero:** `--bg #fbf8ff` canvas; `.eyebrow` uppercase `--muted` → headline Inter 600 `--text-4xl` (`--leading-tight`, tracking `-0.025em`) `--fg` → `.lead` `--fg-2` `--text-xl` → `.btn-primary` (`--accent #7c3aed`) + `.btn-secondary`.
- **Card:** `--surface` fill, `--border` structure, `--radius-lg 24px`, `--elev-raised` when raised; title `--fg` 600, body `--fg-2`.
- **Iteration rules:** (1) bold, contrasting type pairings; (2) accent `#7c3aed` drives interaction; (3) keep type in Inter/Inter; (4) honor the spacing + radius scale exactly.
