...
> Printed-paper UI: off-white stock, ink type, subtle folds, and editorial calm.

## 1. Visual Theme & Atmosphere

Paper design imitates print — off-white stock surfaces, ink-toned type, hairline rules, and a calm editorial rhythm. It reads like a well-set printed page on screen.

**Key characteristics**
- Off-white paper-stock surfaces
- Ink-toned type and hairline rules
- Calm editorial rhythm
- Print-like restraint
...
**Do**
- Use paper-stock surfaces and ink type.
- Rely on hairline rules.
- Keep an editorial rhythm.
- Bind `tokens.css` into `:root` and reference every value via `var(--*)`.

**Don't**
- Don't use glossy digital gradients.
- Don't add glow or neon.
- Don't break the print calm.
- Don't invent off-palette colors or redefine the token contract.

## 10. Agent Prompt Guide

- **Hero:** `--bg #faf3df` canvas; `.eyebrow` uppercase `--muted` → headline Georgia 600 `--text-4xl` (`--leading-tight`, tracking `-0.025em`) `--fg` → `.lead` `--fg-2` `--text-xl` → `.btn-primary` (`--accent #8f5d24`) + `.btn-secondary`.
- **Card:** `--surface` fill, `--border` structure, `--radius-lg 24px`, `--elev-raised` when raised; title `--fg` 600, body `--fg-2`.
- **Iteration rules:** (1) off-white paper-stock surfaces; (2) accent `#8f5d24` drives interaction; (3) keep type in Georgia/Inter; (4) honor the spacing + radius scale exactly.
