...
senweavercoding-landing skill. Several common image-model defaults will
...
See `skills/senweavercoding-landing/assets/imagegen-prompts.md` for the
...
- Always include the metadata strip, the side rails, the Roman
  section rules, and a footer with a giant `SenWeaverCoding.` (or brand)
