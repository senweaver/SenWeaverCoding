...
> Confident, high-impact UI: oversized type, strong color, and decisive contrast.

## 1. Visual Theme & Atmosphere

Bold design commands attention — oversized headlines, strong solid color, and high contrast. Statements are large, hierarchy is dramatic, and the accent is used without hesitation.

**Key characteristics**
- Oversized, heavy display type
- Strong solid color and high contrast
- Dramatic scale jumps for hierarchy
- Decisive, confident accent usage
...
- **Weights:** 400 body, 500 UI/labels, 600–700 headings. Lead with Arial Black for display, set running text in Inter, and use SF Mono for code/technical labels.
...
**Do**
- Use oversized headlines and strong contrast.
- Commit fully to the accent.
- Make scale jumps dramatic.
- Bind `tokens.css` into `:root` and reference every value via `var(--*)`.

**Don't**
- Don't be timid with type or color.
- Don't flatten hierarchy.
- Don't dilute the accent.
- Don't invent off-palette colors or redefine the token contract.

## 10. Agent Prompt Guide

- **Hero:** `--bg #ffffff` canvas; `.eyebrow` uppercase `--muted` → headline Arial Black 600 `--text-4xl` (`--leading-tight`, tracking `-0.025em`) `--fg` → `.lead` `--fg-2` `--text-xl` → `.btn-primary` (`--accent #111111`) + `.btn-secondary`.
- **Card:** `--surface` fill, `--border` structure, `--radius-lg 12px`, `--elev-raised` when raised; title `--fg` 600, body `--fg-2`.
- **Iteration rules:** (1) oversized, heavy display type; (2) accent `#111111` drives interaction; (3) keep type in Arial Black/Inter; (4) honor the spacing + radius scale exactly.
