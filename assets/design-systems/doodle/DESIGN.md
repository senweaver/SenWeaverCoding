...
> Hand-drawn playful UI: sketchy strokes, marker accents, and friendly imperfection.

## 1. Visual Theme & Atmosphere

Doodle design feels hand-drawn — sketchy borders, marker-style accents, wobble and imperfection, and a warm, approachable personality.

**Key characteristics**
- Sketchy hand-drawn strokes
- Marker-style accent color
- Charming imperfection and wobble
...
- **Weights:** 400 body, 500 UI/labels, 600–700 headings. Lead with Comic Sans MS for display, set running text in Comic Sans MS, and use SF Mono for code/technical labels.
...
**Do**
- Use sketchy strokes and marker accents.
- Embrace friendly imperfection.
- Keep it warm and approachable.
- Bind `tokens.css` into `:root` and reference every value via `var(--*)`.

**Don't**
- Don't use rigid perfect geometry.
- Don't go corporate and stiff.
- Don't overload the sketchiness.
- Don't invent off-palette colors or redefine the token contract.

## 10. Agent Prompt Guide

- **Hero:** `--bg #fff8d7` canvas; `.eyebrow` uppercase `--muted` → headline Comic Sans MS 600 `--text-4xl` (`--leading-tight`, tracking `-0.025em`) `--fg` → `.lead` `--fg-2` `--text-xl` → `.btn-primary` (`--accent #ff6b00`) + `.btn-secondary`.
- **Card:** `--surface` fill, `--border` structure, `--radius-lg 24px`, `--elev-raised` when raised; title `--fg` 600, body `--fg-2`.
- **Iteration rules:** (1) sketchy hand-drawn strokes; (2) accent `#ff6b00` drives interaction; (3) keep type in Comic Sans MS/Comic Sans MS; (4) honor the spacing + radius scale exactly.
