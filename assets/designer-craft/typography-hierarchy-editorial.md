...
- **Bind editorial type to the brand's display font.** The dramatic scale
  jumps must use `var(--font-display)` from the active design system; an
  editorial surface set in `system-ui` reads as SaaS, not a publication.
- **Real prose, never filler.** Editorial pacing (rest/tension/disruption)
  cannot be evaluated on Lorem Ipsum. Write or import real article copy so
  the rhythm and pull-quote placement are judged on actual reading flow.
- **Pace with the spacing scale.** Drive the asymmetrical rhythm
  (compression → expansion) off design-system spacing tokens so the
  alternation is intentional and consistent, not random pixel gaps.
