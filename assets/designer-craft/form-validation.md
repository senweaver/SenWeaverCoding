...
- **Ship the no-JS floor in self-contained artifacts.** A canvas form unit
  must carry native constraints (`required`, `type`, `pattern`,
  `minlength`) so it validates even if the inline script doesn't run inside
  the sandboxed iframe. Style off `:user-invalid`, never `:invalid`.
- **Demonstrate the error path, don't just style the happy path.** Seed the
  unit with a visible invalid example (one field in `invalid-after-touched`
  with a real adaptive message and an error summary) so the form's recovery
  design is reviewable in the rendered preview, per `state-coverage.md`.
- **No backend in the preview — fake the server honestly.** When the brief
  has no real endpoint, simulate submit with a short delay + a deterministic
  result and label it as sample behavior; never wire the preview to an
  invented API.
