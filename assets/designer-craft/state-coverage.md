...
- **Optimistic UI with rollback.** For user-initiated mutations (toggle,
  like, reorder, send), update the UI immediately, then reconcile with the
  result. On failure, roll the item back to its prior value *and* surface a
  non-blocking error — never leave the optimistic state lying.
- **Skeletons must match the real layout.** A skeleton that doesn't mirror
  the populated grid causes a layout jump on load. Reserve final
  dimensions (use `aspect-ratio` and fixed row heights) so content swaps in
  without reflow / CLS.
- **Seed every state in a static artifact.** Since canvas units are
  self-contained HTML, ship a tiny state switcher (buttons or `:target`)
  that toggles loading / empty / error / populated, so all five states are
  visible and reviewable in one rendered unit — not just the populated one.
- **Disable, don't hide, the pending control.** On submit, keep the button
  in place with a loading label; hiding it shifts layout and loses the
  user's focus anchor.
