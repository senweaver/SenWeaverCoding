...
(`competitive-ads-extractor`) or via one of its trigger phrases. Its bundled templates,
checklists, and reference files are read on demand with the
`designer_skill_read` tool (`id: competitive-ads-extractor`, `path: <relative file>`) —
