...
function parseInput(input: unknown): Question[] {
  ...
  const fromQuestionLike = (q: unknown): Question | null => {
    ...
    return { question: text, header, options }
  }

  if (Array.isArray(obj.questions)) {
    return obj.questions.map(fromQuestionLike).filter((q): q is Question => !!q)
  }

  if (typeof obj.question === 'string' || typeof obj.prompt === 'string') {
    const single = fromQuestionLike(obj)
