...
import { previewStatusBar } from '../lib/designPreviewData'
    ...
    <div className="bg-[#FAF9F5] text-[#1B1C1A] flex min-h-screen font-[Inter,sans-serif]">
      <aside className="fixed left-0 top-0 h-full w-[280px] bg-[#F4F4F0] flex flex-col p-4 gap-2 text-sm font-medium z-40">
        ...
        <nav className="flex-1 space-y-1">
          ...
          <div className="px-3 py-2 bg-[#FAF9F5] text-[#1B1C1A] rounded-lg relative before:content-[''] before:absolute before:left-[-8px] before:w-1 before:h-4 before:bg-[#A16900] before:rounded-full cursor-pointer duration-200 ease-in-out flex items-center gap-3">
      ...
      <main className="flex-1 ml-[280px] flex flex-col min-h-screen">
        <header className="fixed top-0 right-0 left-[280px] z-30 bg-[#FAF9F5] flex justify-between items-center px-6 h-12 w-full border-b border-[#F4F4F0]">
          <div className="flex items-center gap-6 h-full">
            <div className="text-sm font-bold text-[#1B1C1A] uppercase tracking-tighter font-[Manrope,sans-serif]">SenWeaverCoding</div>
            <nav className="flex items-center gap-4 h-full font-[Manrope,sans-serif] font-semibold tracking-wide text-sm">
              <span className="text-[#87736D] hover:text-[#A16900] transition-colors cursor-pointer active:opacity-70 h-full flex items-center">{t('titlebar.code')}</span>
              <span className="text-[#87736D] hover:text-[#A16900] transition-colors cursor-pointer active:opacity-70 h-full flex items-center">{t('titlebar.terminal')}</span>
              <span className="text-[#1B1C1A] border-b-2 border-[#A16900] pb-1 h-full flex items-center pt-1">{t('titlebar.history')}</span>
            </nav>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div className="text-[#87736D] hover:text-[#A16900] transition-colors cursor-pointer text-xs font-semibold uppercase tracking-wider">{t('sidebar.settings')}</div>
          </div>
        </header>
              ...
              <div className="flex flex-col items-center py-20 px-8">
                <div className="w-32 h-32 rounded-full bg-[#E9E8E4] flex items-center justify-center mb-10 shadow-sm">
                  <div className="w-20 h-20 rounded-full bg-white flex items-center justify-center border border-[#DAC1BA]/10">
                    <span
                      className="material-symbols-outlined text-[#A16900] text-5xl"
                ...
                <button className="group relative px-8 py-4 bg-[#A16900] text-white rounded-xl font-[Manrope,sans-serif] font-bold text-sm tracking-wide shadow-lg hover:shadow-[#A16900]/20 transition-all flex items-center gap-3 overflow-hidden">
        ...
        <footer className="fixed bottom-0 left-0 w-full h-8 bg-[#FAF9F5] flex items-center justify-between px-4 z-50 border-t border-[#87736D]/20 font-[Inter,sans-serif] text-xs tracking-tight">
          <div className="flex items-center gap-3 text-[#87736D]">
            ...
            <span>{previewStatusBar.user} &bull; {previewStatusBar.username} &bull; {previewStatusBar.plan}</span>
          </div>
          <div className="flex items-center gap-6">
            ...
            <span className="text-[#A16900] font-bold hover:bg-[#F4F4F0] px-2 py-0.5 rounded transition-colors cursor-pointer flex items-center gap-1">
