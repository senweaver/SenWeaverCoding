...
export default function SessionControls() {
  ...
  const activeModel = previewModels.find((m) => m.id === selectedModel)
  ...
  return (
    <div className="h-screen w-screen bg-background text-on-surface font-body selection:bg-primary-fixed overflow-hidden relative">
      <header className="bg-[#FAF9F5] font-headline font-semibold tracking-wide text-sm fixed top-0 left-0 right-0 flex justify-between items-center px-6 h-12 z-40">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden md:flex gap-4">
            <a className="text-[#1B1C1A] border-b-2 border-[#A16900] pb-1 cursor-pointer active:opacity-70">
              Code
            </a>
            <a className="text-[#87736D] hover:text-[#A16900] transition-colors cursor-pointer active:opacity-70">
              Terminal
            </a>
            <a className="text-[#87736D] hover:text-[#A16900] transition-colors cursor-pointer active:opacity-70">
              History
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="ml-2 px-3 py-1 bg-surface-container-high rounded text-[#A16900] hover:bg-surface-container-highest transition-colors">
            Settings
          </button>
        </div>
      </header>
      ...
      <aside className="bg-[#F4F4F0] font-body text-sm font-medium fixed left-0 top-0 h-full w-[280px] hidden md:flex flex-col p-4 gap-2 pt-16 z-30">
        <div className="px-2 mb-4">
          <div className="flex items-center gap-3 mb-1">
            <div className="w-8 h-8 rounded-full bg-[var(--color-surface-dim)] flex items-center justify-center">
              <span className="material-symbols-outlined text-[var(--color-outline)] text-sm">folder</span>
            </div>
            <div>
        ...
        <button className="w-full text-left p-2.5 bg-[#FAF9F5] text-[#1B1C1A] rounded-lg relative before:content-[''] before:absolute before:left-[-8px] before:w-1 before:h-4 before:bg-[#A16900] before:rounded-full before:top-1/2 before:-translate-y-1/2 transition-all duration-200 ease-in-out flex items-center gap-3">
          <span className="material-symbols-outlined text-[#A16900]">add</span>
        ...
        <button className="w-full text-left p-2.5 text-[#87736D] hover:bg-[#EBEBE6] rounded-lg transition-all duration-200 ease-in-out flex items-center gap-3" data-count={previewSessions.today.length}>
          <span className="material-symbols-outlined">history</span>
          <span>Today</span>
        </button>
        <button className="w-full text-left p-2.5 text-[#87736D] hover:bg-[#EBEBE6] rounded-lg transition-all duration-200 ease-in-out flex items-center gap-3" data-count={previewSessions.previous7Days.length}>
          <span className="material-symbols-outlined">event_note</span>
          <span>Previous 7 Days</span>
        </button>
        <button className="w-full text-left p-2.5 text-[#87736D] hover:bg-[#EBEBE6] rounded-lg transition-all duration-200 ease-in-out flex items-center gap-3" data-count={previewSessions.older.length}>
            ...
            <div className="col-span-8 space-y-8">
              <div className="bg-surface-container-low rounded-xl p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="font-semibold text-sm">Sonnet 4.6</span>
                </div>
      ...
      <div className="fixed inset-0 z-50 flex flex-col justify-end items-center pointer-events-none p-8">
          ...
          {showPermissions && (
            ...
            >
              ...
              <div className="p-2 space-y-1">
                {previewPermissionModes.map((mode) => {
        ...
        >
          <textarea
            ...
            placeholder="Reply to SenWeaverCoding..."
            rows={2}
          />
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {previewPermissionModes.find((m) => m.id === selectedPermission)?.label ||
                  'Ask permissions'}
              </button>
