...
inventory::submit!(StaticSlashCommand {
    ...
    usage: "/doctor [network]",
    category: CommandCategory::Debug,
    hidden: false,
    requires_interactive: false,
    remote_safe: true,
    handler: make_handler!(handle_doctor),
});

pub async fn handle_doctor(ctx: CommandContext) -> CommandResult {
    ...
    if sen_dir.exists() {
        ...
        checks
            .push("  ⚠ .senweavercoding dir: not found (will be created on first use)".to_string());
    }
    ...
    match crate::bootstrap::try_get_state() {
        ...
        None => {
            checks.push("  ⚠ bootstrap: not initialized (normal on first command)".to_string());
        }
    }
...
async fn handle_network() -> CommandResult {
    ...
    match std::env::var("EXA_API_KEY").ok().filter(|k| !k.is_empty()) {
        Some(ref key) => {
            let body = serde_json::json!({
                "query": "test",
                "numResults": 1,
