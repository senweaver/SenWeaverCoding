...
//! `sen update` — self-update pipeline with rollback.
...
async fn download_binary(url: &str, dest: &Path) -> Result<()> {
    ...
    // Release assets are .tar.gz archives containing a single `sen` binary.
...
/// Extract the `sen` binary from a `.tar.gz` archive.
fn extract_tar_gz(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    for entry in archive.entries().context("failed to read tar entries")? {
        ...
        // The archive contains a single binary named "sen" (or "sen.exe" on Windows).
...
mod tests {
    ...
    fn find_asset_url_picks_correct_gnu_over_android() {
        let release = make_release(&[
            "sen-aarch64-linux-android.tar.gz",
            "sen-aarch64-unknown-linux-gnu.tar.gz",
            "sen-x86_64-unknown-linux-gnu.tar.gz",
            "sen-x86_64-apple-darwin.tar.gz",
            "sen-aarch64-apple-darwin.tar.gz",
        ]);
    ...
    fn extract_tar_gz_finds_binary() {
        ...
        // Build a tar.gz in memory containing a fake "sen" binary.
        ...
        let mut tar_buf = Vec::new();
        {
            ...
            header.set_cksum();
            builder
                .append_data(&mut header, "sen", &fake_binary[..])
                .unwrap();
        ...
        let dest = tmp.path().join("sen_extracted");
    ...
    fn extract_tar_gz_errors_on_missing_binary() {
        ...
        // Build a tar.gz with a file that is NOT named "sen".
        let mut tar_buf = Vec::new();
        {
            let mut builder = tar::Builder::new(&mut tar_buf);
            let mut header = tar::Header::new_gnu();
