...
pub fn command() -> SlashCommand {
    SlashCommand {
        name: "hooks".into(),
        aliases: vec![],
        description: "List and manage session hooks".into(),
        usage: "/hooks [list|add|remove] [args]".into(),
...
pub async fn handle(ctx: CommandContext) -> CommandResult {
    ...
    match sub.as_str() {
        "list" => {
            ...
            lines.push("  webhook_audit -- Audit log via webhook (built-in, enabled in config)".to_string());
            lines.push("  command_logger -- Log all commands (built-in, enabled in config)".to_string());
        ...
        "remove" => {
            let name = ctx.args.get(1).map(|s| s.as_str()).unwrap_or("");
            if name.is_empty() {
                return CommandResult::err("Usage: /hooks remove <hook_name>\nNote: To disable a hook, edit config.toml and restart.");
            }
            CommandResult::ok(format!(
