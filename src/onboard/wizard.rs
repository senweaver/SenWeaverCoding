...
/// Wrapper for safe environment variable operations.
/// In Rust 1.94+, crate::util::set_env_var is unsafe.
#[inline]
fn set_env_var(key: &str, value: impl AsRef<std::ffi::OsStr>) {
    crate::util::set_env_var(key, value.as_ref());
}

/// Wrapper for safe environment variable removal.
#[inline]
...
// Banner used in compact contexts (single line, no box drawing)
...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            set_env_var("SEN_AUTOSTART_CHANNELS", "1");
        }
    }
...
/// Use `sen onboard` or `sen onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".senweavercoding");
    (config_dir.clone(), config_dir.join("workspace"))
}

fn homebrew_prefix_for_exe(exe: &Path) -> Option<&'static str> {
    let exe = exe.to_string_lossy();
    if exe == "/opt/homebrew/bin/sen"
        || exe.starts_with("/opt/homebrew/Cellar/sen/")
        || exe.starts_with("/opt/homebrew/opt/sen/")
    {
        return Some("/opt/homebrew");
    }

    if exe == "/usr/local/bin/sen"
        || exe.starts_with("/usr/local/Cellar/sen/")
        || exe.starts_with("/usr/local/opt/sen/")
    {
...
fn quick_setup_homebrew_service_note(
    ...
    let service_root = Path::new(prefix).join("var").join("sen");
    ...
    Some(format!(
        "Homebrew service note: `brew services` uses {} (config {}) by default. Your onboarding just wrote {}. If you plan to run SenWeaverCoding as a service, copy or link this workspace first.",
...
async fn run_quick_setup_with_home(
    ...
    let (sen_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    let default_ctx = ProjectContext {
        ...
        timezone: "UTC".into(),
        agent_name: "SenWeaverCoding".into(),
        communication_style:
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     sen agent -m \"Hello!\"");
            println!("    2. Gateway:  sen gateway");
            println!("    3. Status:   sen status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             sen auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              sen agent -m \"Hello!\"");
                println!("    3. Gateway:           sen gateway");
                println!("    4. Status:            sen status");
            }
        } else {
            let env_var = provider_env_var(&provider_name);
            println!("    1. Set your API key:  export {env_var}=\"sk-...\"");
            println!("    2. Or edit:           ~/.senweavercoding/config.toml");
            println!("    3. Chat:              sen agent -m \"Hello!\"");
            println!("    4. Gateway:           sen gateway");
        }
    } else {
        println!("    1. Chat:     sen agent -m \"Hello!\"");
        println!("    2. Gateway:  sen gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `sen models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: sen models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("SenWeaverCoding will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how SenWeaverCoding connects to external apps.");
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("SenWeaverCoding can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("SenWeaverCoding can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should SenWeaverCoding interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how SenWeaverCoding stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to SenWeaverCoding from anywhere.");
    ...
    loop {
        ...
        match choice {
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet(
                    "SenWeaverCoding reads your iMessage database and replies via AppleScript.",
                );
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.senweavercoding/state/whatsapp-web/session.db")
                        .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via SEN_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("SenWeaverCoding is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("sen auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: sen auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("sen channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("sen agent -m \"Hello, SenWeaverCoding!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("sen agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("sen status").yellow());
...
mod tests {
    ...
    impl Drop for EnvVarGuard {
        fn drop(&mut self) {
            if let Some(previous) = &self.previous {
                set_env_var(self.key, previous.as_str());
            } else {
    ...
    async fn quick_setup_model_override_persists_to_config_toml() {
        ...
        let _workspace_env = EnvVarGuard::unset("SEN_WORKSPACE");
        let _config_env = EnvVarGuard::unset("SEN_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        ...
        let _config_env = EnvVarGuard::unset("SEN_CONFIG_DIR");
        let tmp = TempDir::new().unwrap();
        let sen_dir = tmp.path().join(".senweavercoding");
        let config_path = sen_dir.join("config.toml");

        tokio::fs::create_dir_all(&sen_dir).await.unwrap();
    ...
    #[tokio::test]
    async fn quick_setup_respects_sen_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("sen-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".senweavercoding").join("config.toml");

        let _workspace_env = EnvVarGuard::set("SEN_WORKSPACE", workspace_dir.as_os_str());
        let _config_env = EnvVarGuard::unset("SEN_CONFIG_DIR");
        ...
        .await
        .expect("quick setup should honor SEN_WORKSPACE");
    ...
    fn homebrew_prefix_for_exe_detects_supported_layouts() {
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/opt/homebrew/bin/sen")),
            Some("/opt/homebrew")
        );
        assert_eq!(
            homebrew_prefix_for_exe(Path::new(
                "/opt/homebrew/Cellar/sen/0.5.0/bin/sen",
            )),
            Some("/opt/homebrew")
        );
        assert_eq!(
            homebrew_prefix_for_exe(Path::new("/usr/local/bin/sen")),
            Some("/usr/local")
        );
        assert_eq!(homebrew_prefix_for_exe(Path::new("/tmp/sen")), None);
    }

    #[test]
    fn quick_setup_homebrew_service_note_mentions_service_workspace() {
        let note = quick_setup_homebrew_service_note(
            Path::new("/Users/alix/.senweavercoding/config.toml"),
            Path::new("/Users/alix/.senweavercoding/workspace"),
            Path::new("/opt/homebrew/bin/sen"),
        )
        .expect("homebrew installs should emit a service workspace note");

        assert!(note.contains("/opt/homebrew/var/sen/workspace"));
        assert!(note.contains("/opt/homebrew/var/sen/config.toml"));
        assert!(note.contains("/Users/alix/.senweavercoding/config.toml"));
    }

    #[test]
    fn quick_setup_homebrew_service_note_skips_matching_service_layout() {
        let service_config = Path::new("/opt/homebrew/var/sen/config.toml");
        let service_workspace = Path::new("/opt/homebrew/var/sen/workspace");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** SenWeaverCoding"),
            "should default agent name to SenWeaverCoding"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let ctx = ProjectContext {
            ...
            agent_name: "SenWeaverCoding-v2".into(),
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("SenWeaverCoding-v2"));
    }
    ...
    async fn scaffold_full_personalization() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            .await
            .unwrap();
        assert!(identity.contains("**Name:** Agent"));

        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("You are **Agent**"));
        ...
        let agents = tokio::fs::read_to_string(tmp.path().join("AGENTS.md"))
            .await
            .unwrap();
        assert!(agents.contains("Agent Personal Assistant"));
        ...
        assert!(bootstrap.contains("Introduce yourself as Agent"));

        let heartbeat = tokio::fs::read_to_string(tmp.path().join("HEARTBEAT.md"))
            .await
            .unwrap();
        assert!(heartbeat.contains("Agent"));
    }
