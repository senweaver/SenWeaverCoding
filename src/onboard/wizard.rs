...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    let config = Config {
        ...
        hands: crate::config::HandsConfig::default(),
        buddy: crate::config::BuddyConfig::default(),
        ...
        agent_runtime: crate::config::schema::AgentRuntimeExtras::default(),
        ...
        teams: crate::config::TeamsConfig::default(),
...
fn print_summary(config: &Config) {
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if provider == "openai-codex" {
            ...
            println!(
                "       {}",
                style("or: sen auth paste-token --provider anthropic --auth-kind authorization")
                    .yellow()
            );
        } else {
