...
//! Safe wrappers for environment variable operations.
//!
//! In Rust 1.94+, `std::env::set_var` and `std::env::remove_var` are unsafe.
//! This module provides safe wrappers for use within the application.

use std::ffi::OsStr;

/// Set an environment variable safely.
///
/// # Safety
/// This function is safe to call with application-controlled keys.
/// The caller should ensure the key does not conflict with system variables.
#[inline]
pub fn set_env_var<K: AsRef<OsStr>, V: AsRef<OsStr>>(key: K, value: V) {
    // SAFETY: Setting environment variables is safe when:
    // 1. We only set our own application variables (prefixed appropriately)
    // 2. We don't modify system-critical variables
    unsafe { std::env::set_var(key.as_ref(), value.as_ref()) }
}

/// Remove an environment variable safely.
///
/// # Safety
/// This function is safe to call with application-controlled keys.
#[inline]
pub fn remove_env_var<K: AsRef<OsStr>>(key: K) {
    // SAFETY: Removing environment variables is safe when:
    // 1. We only remove our own application variables
    // 2. We don't modify system-critical variables
...
/// # Examples
/// ```ignore
/// use senweavercoding::util::truncate_with_ellipsis;
///
