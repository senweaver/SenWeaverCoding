...
pub async fn run(config: Config, host: String, port: u16) -> Result<()> {
    ...
    // Initialize the global event bus early so all supervised components
    // (channels, heartbeat, scheduler) can publish events immediately.
    // gateway::run_gateway also calls init_global_bus, but the global
    // LazyLock+RwLock makes this idempotent.
    ...
    // RPC server for external interop (Python SDK, multi-language IPC)
