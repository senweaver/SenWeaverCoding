...
mod tests {
    ...
    #[test]
    fn matrix_store_dir_is_derived_from_sen_dir() {
        let ch = MatrixChannel::new_with_session_hint_and_sen_dir(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
            vec![],
            None,
            None,
            Some(PathBuf::from("/tmp/sen")),
        );

        assert_eq!(
            ch.matrix_store_dir(),
            Some(PathBuf::from("/tmp/sen/state/matrix"))
        );
    }

    #[test]
    fn matrix_store_dir_absent_without_sen_dir() {
        let ch = MatrixChannel::new_with_session_hint(
            "https://matrix.org".to_string(),
            "tok".to_string(),
            "!r:m".to_string(),
