...
async fn transcribe_discord_audio_attachments(
    attachments: &[serde_json::Value],
    client: &reqwest::Client,
    manager: &super::super::pipeline::transcription::TranscriptionManager,
) -> String {
    let mut parts: Vec<String> = Vec::new();
    for att in attachments {
        let ct = att
...
impl Channel for DiscordChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        let raw_content = super::super::strip_tool_call_tags(&message.content);
    ...
    async fn finalize_draft(
        ...
        let text = &super::super::strip_tool_call_tags(text);
