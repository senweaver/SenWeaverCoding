...
impl Channel for WebhookChannel {
    ...
    async fn listen(&self, tx: tokio::sync::mpsc::Sender<ChannelMessage>) -> Result<()> {
            ...
            if let Some(ref secret) = state.secret {
                ...
                let valid = if let Some(sig) = signature {
                    if let Ok(decoded) = hex::decode(sig.trim_start_matches("sha256=")) {
                        ...
                        // SECURITY: Fail closed on invalid hex signature rather than silently accepting
                        tracing::warn!(
        ...
        // SECURITY NOTE: Webhook binds to 0.0.0.0 (all interfaces). For production:
        // 1. Use TLS/HTTPS in front of this service (e.g., nginx, cloudflare)
        // 2. Consider binding to 127.0.0.1 if only local access is needed
        // 3. Ensure firewall rules restrict access to the webhook endpoint
...
mod tests {
    ...
    fn incoming_payload_deserializes_all_fields() {
        let json = r#"{"sender": "sen_user", "content": "hello", "thread_id": "t1"}"#;
        let payload: IncomingWebhook = serde_json::from_str(json).unwrap();
        assert_eq!(payload.sender, "sen_user");
