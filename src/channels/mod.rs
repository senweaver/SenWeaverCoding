...
struct ChannelRuntimeContext {
    ...
    /// When `[rbac] enabled`, tool calls in this channel runtime are checked per sender.
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(sen_dir) = path.parent() {
...
pub fn build_system_prompt_with_mode_and_autonomy(
    ...
    if prompt.is_empty() {
        "You are SenWeaverCoding, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct."
            .to_string()
    } else {
        prompt
    }
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `sen onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `sen daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `sen service stop && sen service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.senweavercoding.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.senweavercoding.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.senweavercoding.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.senweavercoding.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/sen");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: sen channel start");
            println!("To check health:    sen channel doctor");
            println!("To configure:      sen onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `sen onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.senweavercoding/config.toml directly");
        }
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `sen onboard` first.");
        return Ok(());
    }

    println!("🩺 SenWeaverCoding Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `sen gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    // Inject experience replay prompt (multi-agent learning)
    if config.experience.enabled {
        let replay = crate::agent::experience::ExperienceReplay::new(&config.experience);
        if let Some(exp_text) = replay.prompt_injection(None) {
            system_prompt.push_str(&exp_text);
        }
    }

    // Inject skill evolution guidance
    {
        let skill_engine =
            crate::agent::skill_evolution::SkillEvolutionEngine::new(&config.skill_evolution);
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `sen onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 SenWeaverCoding Channel Server");
    println!("  🤖 Model:    {model}");
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        rbac_engine: if config.rbac.enabled {
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                sen_dir: Some(temp.path().to_path_buf()),
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(
            prompt.contains("Name: SenWeaverCoding"),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from SenWeaverCoding 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
This is an example JSON object for profile settings."#;
    ...
    fn aieos_identity_from_inline() {
        ...
        let config = IdentityConfig {
            format: "aieos".into(),
            aieos_path: None,
            aieos_inline: Some(r#"{"identity":{"names":{"first":"Agent"}}}"#.into()),
        };
        ...
        assert!(prompt.contains("**Name:** Agent"));
        assert!(prompt.contains("## Identity"));
    }
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "sen.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "sen.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["sen", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["sen", "restart"]);
    }
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_chat-photo_sen_user")
    ...
    async fn e2e_failed_non_retryable_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_chat-format_sen_user")
            .expect("history should exist for sender");
        assert_eq!(turns.len(), 2);
        assert_eq!(turns[0].role, "user");
        assert_eq!(turns[0].content, "What is WAL?");
    ...
    fn is_stop_command_matches_with_bot_suffix() {
        assert!(is_stop_command("/stop@sen_bot"));
    }
