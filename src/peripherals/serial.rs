...
async fn send_request(port: &mut SerialStream, cmd: &str, args: Value) -> anyhow::Result<Value> {
    ...
    // PERFORMANCE: Limit response buffer to prevent memory exhaustion.
    // Large responses are truncated to MAX_RESPONSE_SIZE bytes.
    const MAX_RESPONSE_SIZE: usize = 16_384; // 16KB max
