...
/// SenWeaverCoding Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
...
/// Flash SenWeaverCoding firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("sen_flash_{}", uuid::Uuid::new_v4()));
    ...
    println!("SenWeaverCoding firmware flashed successfully.");
