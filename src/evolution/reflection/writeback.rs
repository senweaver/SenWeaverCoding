...
use super::lesson::ReflectionLesson;
...
pub async fn apply_writeback(
    ...
    let mut budget_exhausted_reported = false;
                ...
                ReflectionWritebackTarget::Lessons => {
                    if budget == 0 {
                        if !budget_exhausted_reported {
                            report.errors.push("lesson_budget_exhausted".to_string());
...
fn write_to_lessons_table(
    ...
    if store.lesson_duplicate_exists(&lesson.title, &lesson.body)? {
        return Ok(false);
    }
    let day_ago_ms = (Utc::now() - chrono::Duration::hours(24)).timestamp_millis();
    if store.lessons_created_since(day_ago_ms).unwrap_or(0)
...
fn render_rule_markdown(lesson: &ReflectionLesson) -> String {
    ...
    buf.push_str("---\n");
    buf.push_str(&format!("name: {}\n", lesson.title));
    buf.push_str("alwaysApply: false\n");
    buf.push_str(&format!(
        "description: reflection {} (auto-generated draft; review and set alwaysApply manually)\n",
        lesson.kind.as_str()
    ));
    if !lesson.tags.is_empty() {
        buf.push_str(&format!("tags: [{}]\n", lesson.tags.join(", ")));
    ...
    buf.push_str(&format!("# {}\n\n", lesson.title));
    buf.push_str(&format!(
        "_Generated by self-reflection at {}. This rule stays inactive until a human \
         reviews it and enables alwaysApply._\n\n",
...
fn slugify(input: &str) -> String {
    ...
    if clipped.is_empty() {
        ...
        clipped = format!("lesson-{:016x}", std::hash::Hasher::finish(&hasher));
    }
    clipped
}
