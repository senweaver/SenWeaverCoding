...
/// Manages device datasheet files in `~/.senweavercoding/hardware/datasheets/`.
...
impl DatasheetManager {
    /// Create a manager rooted at the default SenWeaverCoding datasheets directory.
    pub fn new() -> Option<Self> {
        let home = directories::BaseDirs::new()?.home_dir().to_path_buf();
        Some(Self {
            datasheet_dir: home.join(".senweavercoding").join("hardware").join("datasheets"),
        })
    }
    ...
    ///
    /// The file is saved as `~/.senweavercoding/hardware/datasheets/<device_name>.pdf`.
...
impl Default for DatasheetManager {
    fn default() -> Self {
        Self::new().unwrap_or_else(|| Self {
            datasheet_dir: PathBuf::from(".senweavercoding/hardware/datasheets"),
        })
    }
}
        ...
        match action.as_str() {
                    ...
                    Ok(path) => Ok(ToolResult {
                        ...
                        output: format!(
                            "Datasheet for '{device}' downloaded successfully.\n\
                             ...
                             ~/.senweavercoding/hardware/devices/aardvark0.md with the key \
            ...
            "list" => {
                ...
                let output = if datasheets.is_empty() {
                    ...
                    format!(
                        "{} cached datasheet(s) in ~/.senweavercoding/hardware/datasheets/:\n{}",
                        datasheets.len(),
                        datasheets
                            .iter()
                            .map(|n| format!("  - {n}"))
