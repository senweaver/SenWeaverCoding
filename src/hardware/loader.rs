...
/// Scan `~/.senweavercoding/tools/` and return all valid plugins.
///
...
/// Return the path `~/.senweavercoding/tools/` using the `directories` crate.
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.senweavercoding/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_returns_path_ending_in_sen_tools() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(".senweavercoding").join("tools");
