...
pub struct HardwareBootResult {
    ...
    /// Content of `~/.senweavercoding/hardware/` context files (HARDWARE.md, device
    /// profiles, and skills) for injection into the system prompt.
    pub context_files_prompt: String,
}

/// Load hardware context files from `~/.senweavercoding/hardware/` and return them
/// concatenated as a single markdown string ready for system-prompt injection.
///
/// Reads (if they exist):
/// 1. `~/.senweavercoding/hardware/HARDWARE.md`
/// 2. `~/.senweavercoding/hardware/devices/<alias>.md` for each discovered alias
/// 3. All `~/.senweavercoding/hardware/skills/*.md` files (sorted by name)
///
...
fn inject_rpi_context(
    ...
    if let Some(ctx) = rpi::RpiSystemContext::discover() {
        ...
        // PERFORMANCE: Using println! for stdout output (user-facing registry messages).
        // tracing! is used for structured logs (debug/info level).
        ...
        if ctx.gpio_available {
            ...
            // User-facing startup messages for tool registration
...
///
/// Without the feature: loads plugin tools from `~/.senweavercoding/tools/` only,
...
pub async fn boot(
    ...
    // register each one with AardvarkTransport + full I2C/SPI/GPIO capabilities.
    {
        ...
        for (i, &port) in aardvark_ports.iter().enumerate() {
            ...
            // User-facing stdout message for hardware registration
...
/// Handle `sen hardware` subcommands.
...
mod tests {
    ...
    fn hardware_context_contains_device_exec_rule() {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|d| d.home_dir().to_path_buf()) {
            let hw_md = home.join(".senweavercoding").join("hardware").join("HARDWARE.md");
            if hw_md.exists() {
