...
/// SenWeaverCoding serial protocol handler — written to the Pico after MicroPython boots.
...
/// Ensure `~/.senweavercoding/firmware/pico/` exists and contains the bundled assets.
///
...
mod tests {
    ...
    #[test]
    fn pico_main_py_contains_sen_marker() {
        let src = std::str::from_utf8(PICO_MAIN_PY).expect("main.py is not valid UTF-8");
        assert!(
            src.contains("sen"),
            "main.py should contain 'sen' firmware marker"
        );
    }
