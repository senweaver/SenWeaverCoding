...
/// Default baud rate for SenWeaverCoding serial devices.
...
/// Serial transport for SenWeaverCoding hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify SenWeaverCoding firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "sen"`.
    ///
    /// Returns `true` if a SenWeaverCoding device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "sen" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
