...
// -- Modules ported from claude-code-typescript-src--
pub mod bootstrap;
pub mod bridge;
pub mod buddy;
pub mod constants;
...
pub use entrypoints::{
    HookEvent, PermissionMode, SdkConfig, SdkEntrypoint, SdkHookCallback, SdkMessage,
    SdkMcpServer, SdkModelUsage, SdkSession, SdkStatus, SdkToolCall, SdkToolCallBuilder,
    SdkTurnEvent, SdkTurnResult,
};
...
pub enum SkillCommands {
    ...
    /// Search remote skill sources (GitHub, ClawHub, HuggingFace)
    Search {
        /// Search query to filter discovered skills
        query: String,
    },
}
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `SenWeaverCoding` workspace
    Openclaw {
...
Examples:
  ...
  sen peripheral add esp32 /dev/ttyUSB0")]
    ...
    /// Flash SenWeaverCoding firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
...
Examples:
  ...
  sen peripheral flash -p COM3")]
    ...
    /// Flash SenWeaverCoding firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
