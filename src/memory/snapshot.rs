...
pub fn export_snapshot(workspace_dir: &Path) -> Result<usize> {
    ...
    // SECURITY: Handle I/O errors explicitly rather than panicking.
    // Log and return error if header write fails.
...
pub fn hydrate_from_snapshot(workspace_dir: &Path) -> Result<usize> {
    ...
    // Initialize schema (must match SqliteMemory::init_schema)
    conn.execute_batch(
...
mod tests {
    ...
    fn parse_snapshot_basic() {
        let input = r#"# 🧠 SenWeaverCoding Memory Snapshot

> Auto-generated by SenWeaverCoding.
...
I am SenWeaverCoding, a self-preserving AI agent.
...
"#;
    ...
    fn parse_snapshot_empty() {
        let input = "# 🧠 SenWeaverCoding Memory Snapshot\n\n> Nothing here.\n";
        let entries = parse_snapshot(input);
