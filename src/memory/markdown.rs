...
impl MarkdownMemory {
    ...
    // Known limitation: read-then-write is not atomic (TOCTOU race). Concurrent
    // appends may lose data. Proper mitigation requires advisory file locking
    // (e.g. `fs2::FileExt`), which is deferred until multi-process access is needed.
