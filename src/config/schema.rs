...
/// Top-level SenWeaverCoding configuration, loaded from `config.toml`.
///
/// Resolution order: `SEN_WORKSPACE` env → `active_workspace.toml` marker → `~/.senweavercoding/config.toml`.
...
pub struct Config {
    ...
    /// Env: `SEN_API_KEY` / `API_KEY` (most providers); for SenWeaver use `SENWEAVER_API_KEY`
    /// (generic `API_KEY` is intentionally ignored when `default_provider` is `senweaver` / `sw`).
    ...
    ///
    /// Can also be set via `SEN_EXTRA_HEADERS` environment variable using
    ...
    /// RPC server configuration for external interop (`[rpc]`).
    #[serde(default)]
    pub rpc: RpcConfig,
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `SEN_LOCALE`,
    ...
    /// Guardrails configuration for pre-tool authorization (`[guardrails]`).
    #[serde(default)]
    pub guardrails: crate::guardrails::GuardrailsConfig,

    /// Plan mode configuration for structured task planning (`[plan_mode]`).
    #[serde(default)]
    pub plan_mode: crate::agent::plan_mode::PlanModeConfig,

    /// Auto-title configuration for session title generation (`[auto_title]`).
    #[serde(default)]
    pub auto_title: crate::agent::auto_title::AutoTitleConfig,

    /// Suggestions engine configuration (`[suggestions]`).
    #[serde(default)]
    ...
    /// User profile personalization configuration (`[user_profile]`).
    #[serde(default)]
    pub user_profile: crate::agent::user_profile::UserProfileConfig,

    /// LLM-as-judge self-evaluation configuration (`[self_eval]`).
    #[serde(default)]
    pub self_eval: crate::agent::self_eval::SelfEvalConfig,

    /// Feedback collection and reward aggregation (`[feedback]`).
    #[serde(default)]
    pub feedback: crate::agent::feedback::FeedbackConfig,

    /// Experience replay for few-shot learning (`[experience]`).
    #[serde(default)]
    pub experience: crate::agent::experience::ExperienceConfig,

    /// Self-reflection loop for behavioral insights (`[self_reflection]`).
    #[serde(default)]
    pub self_reflection: crate::agent::self_reflection::SelfReflectionConfig,

    /// Adaptive prompt optimization from feedback (`[prompt_optimizer]`).
    #[serde(default)]
    ...
    /// RBAC (Role-Based Access Control) configuration (`[rbac]`).
    #[serde(default)]
    pub rbac: crate::security::rbac::RbacConfig,

    /// Tool output compression for token savings (`[tool_output_compressor]`).
    #[serde(default)]
    pub tool_output_compressor: crate::agent::tool_output_compressor::ToolOutputCompressorConfig,

    /// Token budget management and allocation (`[token_budget]`).
    #[serde(default)]
...
pub struct AgentConfig {
    ...
    /// Global agent directives - default instructions applied to all agent sessions.
    /// These can be overridden by project-level AGENTS.md files.
    #[serde(default)]
    pub global_directives: Vec<GlobalDirective>,

    /// Project-level configuration directory for per-project settings.
    /// When set, sen will look for .sen.toml or sen.toml in project root.
    #[serde(default)]
    pub project_config_dir: Option<String>,

    /// Auto-indexing configuration for project intelligence.
    /// When enabled, indexes code automatically for better context awareness.
    #[serde(default)]
    pub auto_index: AutoIndexConfig,
}

/// A global directive applied to agent sessions.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct GlobalDirective {
    /// The directive content (instruction text).
    pub content: String,
    /// Optional: only apply this directive when mode matches.
    #[serde(default)]
...
pub struct AutoIndexConfig {
    /// Enable automatic code indexing. Default: false.
    #[serde(default)]
    pub enabled: bool,
    /// File patterns to include in indexing. Default: ["**/*.rs", "**/*.ts", "**/*.js"].
    #[serde(default = "default_auto_index_include_patterns")]
    pub include_patterns: Vec<String>,
    /// File patterns to exclude from indexing. Default: ["**/node_modules/**", "**/target/**"].
    #[serde(default = "default_auto_index_exclude_patterns")]
    pub exclude_patterns: Vec<String>,
    /// Maximum files to index. Default: 10000.
    #[serde(default = "default_auto_index_max_files")]
    pub max_files: usize,
    /// Index refresh interval in seconds. Default: 3600 (1 hour).
...
pub struct SkillsConfig {
    ...
    /// List of skill names that are disabled and will not be loaded.
    #[serde(default)]
...
pub struct RpcConfig {
    /// Enable the RPC server. Default: true.
    #[serde(default = "default_rpc_enabled")]
    pub enabled: bool,

    /// Enable stdio transport (for IDE integration / subprocess). Default: true.
    #[serde(default = "default_rpc_stdio")]
    pub stdio: bool,

    /// Enable Unix Domain Socket transport (local clients, Unix-only).
    /// Windows falls back to stdio + HTTP. Default: false.
    #[serde(default)]
    pub unix_socket: Option<String>,

    /// Enable HTTP transport (network clients / microservices). Default: false.
    #[serde(default)]
    pub http: Option<RpcHttpConfig>,

    /// Session idle timeout in seconds. Sessions not touched within this
    /// window are reaped by the background reaper. Default: 300 (5 min).
...
/// HTTP transport configuration for the RPC server.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct RpcHttpConfig {
    ...
    /// Port to listen on. Default: 42618.
...
fn default_rpc_stdio() -> bool {
    true
}

fn default_rpc_session_timeout() -> u64 {
    300
}

fn default_rpc_max_sessions() -> usize {
    100
}

fn default_rpc_http_host() -> String {
    "127.0.0.1".into()
}
...
pub struct WebSearchConfig {
    ...
    /// Tavily Search API key (optional, for AI-optimised search)
    #[serde(default)]
    pub tavily_api_key: Option<String>,
    /// Exa API key (optional, for semantic search)
    #[serde(default)]
...
/// execution events back to SenWeaverCoding's gateway, updating a Slack message
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all SenWeaverCoding-managed HTTP traffic (default).
    #[default]
    Internal,
...
fn set_proxy_env_pair(key: &str, value: Option<&str>) {
    ...
    if let Some(value) = value.and_then(|candidate| normalize_proxy_url_option(Some(candidate))) {
        crate::util::set_env_var(key, &value);
        crate::util::set_env_var(&lowercase_key, &value);
    } else {
        crate::util::remove_env_var(key);
        crate::util::remove_env_var(&lowercase_key);
    }
}

fn clear_proxy_env_pair(key: &str) {
    crate::util::remove_env_var(key);
    crate::util::remove_env_var(&key.to_ascii_lowercase());
}
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "sen_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "sen".
    #[serde(default)]
...
pub struct RuntimeConfig {
    ...
    /// WASM sandbox settings (used when `kind = "wasm"` or as sandbox backend).
    #[serde(default)]
...
pub struct WasmRuntimeConfig {
    /// Fuel limit per module invocation (each instruction costs 1 fuel).
    /// Prevents infinite loops. Default: 1,000,000.
    #[serde(default = "default_wasm_fuel_limit")]
    pub fuel_limit: u64,

    /// Memory limit in megabytes per module. Default: 64.
    ...
    /// Allow WASM modules to read files from the workspace.
    #[serde(default)]
    pub allow_workspace_read: bool,

    /// Allow WASM modules to write files to the workspace.
    #[serde(default)]
    pub allow_workspace_write: bool,

    /// HTTP hosts that WASM modules are allowed to contact (empty = no network).
    #[serde(default)]
...
pub struct MatrixConfig {
    ...
    /// When set, SenWeaverCoding recovers room keys and cross-signing secrets on startup.
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `SEN_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
    ...
    /// Example: `["@?SenWeaverCoding", "\\+?15555550123"]`
    #[serde(default)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `SEN_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
    ...
    #[serde(default)]
    pub proxy_url: Option<String>,
    /// Display name of the bot in Nextcloud Talk (e.g. "sen").
...
/// How SenWeaverCoding receives events from Feishu / Lark.
///
...
pub struct WebAuthnConfig {
    ...
    /// Relying Party display name. Default: "SenWeaverCoding".
...
///
/// When `enabled` is true, SenWeaverCoding validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to SenWeaverCoding permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to SenWeaverCoding tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
...
pub enum SandboxBackend {
    ...
    SandboxExec,
    /// WASM-based in-process sandbox (wasmi interpreter)
    Wasm,
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to sen dir)
...
/// `SEN_CONFIG_DIR` > `SEN_WORKSPACE` > active workspace marker > defaults.
...
async fn resolve_runtime_config_dirs(
    ...
    if let Some((sen_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_sen_dir).await?
    {
        return Ok((
            sen_dir,
            workspace_dir,
...
/// Parse the `SEN_EXTRA_HEADERS` environment variable value.
///
...
///
/// When the workspace is created outside of `sen onboard` (e.g., non-tty
...
impl Config {
    ...
    pub fn apply_env_overrides(&mut self) {
        // ── Provider override (must run FIRST so API key resolution sees the right provider) ──
        // 1) SEN_PROVIDER always wins when set.
        // 2) SEN_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        ...
        let senweaver_like = self.default_provider.as_deref().is_some_and(|p| {
            p.eq_ignore_ascii_case("senweaver") || p.eq_ignore_ascii_case("sw")
        });

        // ── API Key resolution (runs AFTER provider so provider-specific keys match) ──
        // SenWeaver first: never let a stray global `API_KEY` (often another vendor)
        // populate `api_key` when this provider is selected.
        if senweaver_like {
        ...
        // Generic: SEN_API_KEY or API_KEY (skipped for SenWeaver — use SENWEAVER_API_KEY)
        if !senweaver_like {
        ...
        // GLM_API_KEY overrides when provider is a GLM/Zhipu variant.
        ...
        // ZAI_API_KEY overrides when provider is a Z.AI variant.
        ...
        // Provider HTTP timeout: SEN_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("SEN_PROVIDER_TIMEOUT_SECS") {
        ...
        // Extra provider headers: SEN_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        ...
        // Workspace directory: SEN_WORKSPACE
        ...
        // Open-skills opt-in flag: SEN_OPEN_SKILLS_ENABLED
        ...
        // Open-skills directory override: SEN_OPEN_SKILLS_DIR
        ...
        // Skills script-file audit override: SEN_SKILLS_ALLOW_SCRIPTS
        ...
        // Skills prompt mode override: SEN_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("SEN_SKILLS_PROMPT_MODE") {
            if !mode.trim().is_empty() {
                if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                    ...
                    tracing::warn!(
                        "Ignoring invalid SEN_SKILLS_PROMPT_MODE (valid: full|compact)"
                    );
                }
            }
        }

        // Gateway port: SEN_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("SEN_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Gateway host: SEN_GATEWAY_HOST or HOST
        ...
        // Reasoning override: SEN_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("SEN_REASONING_ENABLED")
        ...
        // Web search enabled: SEN_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("SEN_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: SEN_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        ...
        // Brave API key: SEN_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
        ...
        // SearXNG instance URL: SEN_SEARXNG_INSTANCE_URL or SEARXNG_INSTANCE_URL
        ...
        // Web search max results: SEN_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        ...
        // Web search timeout: SEN_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        ...
        // Storage provider key (optional backend override): SEN_STORAGE_PROVIDER
        ...
        // Storage connection URL (for remote backends): SEN_STORAGE_DB_URL
        ...
        // Storage connect timeout: SEN_STORAGE_CONNECT_TIMEOUT_SECS
        ...
        // Proxy enabled flag: SEN_PROXY_ENABLED
        ...
        // Proxy URLs: SEN_* wins, then generic *PROXY vars.
        ...
        if let Ok(no_proxy) =
            std::env::var("SEN_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
...
async fn sync_directory(path: &Path) -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        // Best-effort on Windows: NTFS provides sufficient crash consistency
        // and directory fsync often requires elevated privileges.
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.senweavercoding");
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/sen"));
        assert!(msg.contains("/etc/sen"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("sen"));
    }
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:sen");
        assert_eq!(headers.len(), 2);
        assert_eq!(
            headers[0],
            ("User-Agent".to_string(), "MyApp/1.0".to_string())
        );
        assert_eq!(headers[1], ("X-Title".to_string(), "sen".to_string()));
    }

    #[test]
    async fn parse_extra_headers_env_with_url_value() {
        let headers = parse_extra_headers_env("HTTP-Referer:https://github.com/senweaver/sen");
        assert_eq!(headers.len(), 1);
        // Only splits on first colon, preserving URL colons in value
        assert_eq!(headers[0].0, "HTTP-Referer");
        assert_eq!(headers[0].1, "https://github.com/senweaver/sen");
    }
    ...
    async fn parse_extra_headers_env_whitespace_trimming() {
        let headers = parse_extra_headers_env("  X-Title : sen , User-Agent : cli/1.0 ");
        assert_eq!(headers.len(), 2);
        assert_eq!(headers[0], ("X-Title".to_string(), "sen".to_string()));
        assert_eq!(
            headers[1],
    ...
    async fn parse_extra_headers_env_trailing_comma() {
        let headers = parse_extra_headers_env("X-Title:sen,");
        assert_eq!(headers.len(), 1);
        assert_eq!(headers[0], ("X-Title".to_string(), "sen".to_string()));
    }
...
"#;
        ...
        assert_eq!(parsed.extra_headers.get("X-Title").unwrap(), "sen");
    }
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("sen_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("sen_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.senweavercoding/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
            mode: WhatsAppWebMode::default(),
    ...
    async fn checklist_gateway_serde_roundtrip() {
        ...
        assert_eq!(parsed.path_prefix.as_deref(), Some("/sen"));
...
"#;
    ...
    async fn env_override_api_key() {
        ...
        // Isolate from developer/CI shell: provider + SenWeaver env can flip api_key logic.
        for key in [
            "SEN_PROVIDER",
            "SEN_MODEL_PROVIDER",
            "MODEL_PROVIDER",
        ...
        crate::util::set_env_var("SEN_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        crate::util::remove_env_var("SEN_API_KEY");
    }
    ...
    async fn env_override_api_key_fallback() {
        ...
        crate::util::remove_env_var("SEN_API_KEY");
        crate::util::set_env_var("API_KEY", "sk-fallback-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-fallback-key"));

        crate::util::remove_env_var("API_KEY");
    }
    ...
    async fn env_override_provider() {
        ...
        crate::util::set_env_var("SEN_PROVIDER", "anthropic");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        crate::util::remove_env_var("SEN_PROVIDER");
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        crate::util::remove_env_var("SEN_PROVIDER");
        crate::util::set_env_var("SEN_MODEL_PROVIDER", "openai-codex");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        crate::util::remove_env_var("SEN_MODEL_PROVIDER");
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        crate::util::set_env_var("SEN_OPEN_SKILLS_ENABLED", "true");
        crate::util::set_env_var("SEN_OPEN_SKILLS_DIR", "/tmp/open-skills");
        crate::util::set_env_var("SEN_SKILLS_ALLOW_SCRIPTS", "yes");
        crate::util::set_env_var("SEN_SKILLS_PROMPT_MODE", "compact");
        config.apply_env_overrides();
        ...
        crate::util::remove_env_var("SEN_OPEN_SKILLS_ENABLED");
        crate::util::remove_env_var("SEN_OPEN_SKILLS_DIR");
        crate::util::remove_env_var("SEN_SKILLS_ALLOW_SCRIPTS");
        crate::util::remove_env_var("SEN_SKILLS_PROMPT_MODE");
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        crate::util::set_env_var("SEN_OPEN_SKILLS_ENABLED", "maybe");
        crate::util::set_env_var("SEN_SKILLS_ALLOW_SCRIPTS", "maybe");
        crate::util::set_env_var("SEN_SKILLS_PROMPT_MODE", "invalid");
        config.apply_env_overrides();
    ...
    async fn env_override_provider_fallback() {
        ...
        crate::util::remove_env_var("SEN_PROVIDER");
        crate::util::set_env_var("PROVIDER", "openai");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai"));

        crate::util::remove_env_var("PROVIDER");
    }
    ...
    async fn env_override_provider_fallback_does_not_replace_non_default_provider() {
        ...
        crate::util::remove_env_var("SEN_PROVIDER");
        crate::util::set_env_var("PROVIDER", "openrouter");
        config.apply_env_overrides();
        assert_eq!(
    ...
    #[test]
    async fn env_override_senweaver_provider_overrides_non_default_provider() {
        let _env_guard = env_override_lock().await;
        let mut config = Config {
            default_provider: Some("custom:https://proxy.example.com/v1".to_string()),
            ..Config::default()
        ...
        crate::util::set_env_var("SEN_PROVIDER", "openrouter");
        crate::util::set_env_var("PROVIDER", "anthropic");
        config.apply_env_overrides();
    ...
    async fn env_override_glm_api_key_for_regional_aliases() {
        ...
        crate::util::set_env_var("GLM_API_KEY", "glm-regional-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("glm-regional-key"));

        crate::util::remove_env_var("GLM_API_KEY");
    }
    ...
    async fn env_override_zai_api_key_for_regional_aliases() {
        ...
        crate::util::set_env_var("ZAI_API_KEY", "zai-regional-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("zai-regional-key"));

        crate::util::remove_env_var("ZAI_API_KEY");
    }
    ...
    async fn env_override_model() {
        ...
        crate::util::set_env_var("SEN_MODEL", "gpt-4o");
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        crate::util::remove_env_var("SEN_MODEL");
    }
    ...
    async fn model_provider_profile_responses_uses_openai_codex_and_openai_key() {
        ...
        crate::util::set_env_var("OPENAI_API_KEY", "sk-test-codex-key");
        config.apply_env_overrides();
        crate::util::remove_env_var("OPENAI_API_KEY");
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("sen_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".senweavercoding").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        crate::util::set_env_var("HOME", &temp_home);
        crate::util::set_env_var("SEN_WORKSPACE", &workspace_dir);
        ...
        crate::util::remove_env_var("SEN_WORKSPACE");
        if let Some(home) = original_home {
            crate::util::set_env_var("HOME", home);
        } else {
            crate::util::remove_env_var("HOME");
        }
    ...
    async fn validate_ollama_cloud_model_accepts_remote_endpoint_and_env_key() {
        ...
        crate::util::set_env_var("OLLAMA_API_KEY", "ollama-env-key");
        let result = config.validate();
        crate::util::remove_env_var("OLLAMA_API_KEY");
    ...
    async fn env_override_model_fallback() {
        ...
        crate::util::remove_env_var("SEN_MODEL");
        crate::util::set_env_var("MODEL", "anthropic/claude-3.5-sonnet");
        config.apply_env_overrides();
        assert_eq!(
        ...
        crate::util::remove_env_var("MODEL");
    }
    ...
    async fn env_override_workspace() {
        ...
        crate::util::set_env_var("SEN_WORKSPACE", "/custom/workspace");
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        crate::util::set_env_var("SEN_CONFIG_DIR", &explicit_config_dir);
        crate::util::remove_env_var("SEN_WORKSPACE");
        ...
        crate::util::remove_env_var("SEN_CONFIG_DIR");
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("sen_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".senweavercoding").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".senweavercoding");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("sen_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".senweavercoding");
    ...
    async fn load_or_init_uses_persisted_active_workspace_marker() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("sen_test_home_{}", uuid::Uuid::new_v4()));
        let temp_default_dir = temp_home.join(".senweavercoding");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        let original_home = std::env::var("HOME").ok();
        crate::util::set_env_var("HOME", &temp_home);
        crate::util::set_env_var("SEN_WORKSPACE", &env_workspace_dir);
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        let temp_home =
            std::env::temp_dir().join(format!("sen_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".senweavercoding");
...
"#,
    ...
    async fn env_override_empty_values_ignored() {
        ...
        crate::util::set_env_var("SEN_PROVIDER", "");
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        crate::util::set_env_var("SEN_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        crate::util::remove_env_var("SEN_GATEWAY_PORT");
    }
    ...
    async fn env_override_port_fallback() {
        ...
        crate::util::remove_env_var("SEN_GATEWAY_PORT");
        crate::util::set_env_var("PORT", "9000");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 9000);

        crate::util::remove_env_var("PORT");
    }
    ...
    async fn env_override_gateway_host() {
        ...
        crate::util::set_env_var("SEN_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        crate::util::remove_env_var("SEN_GATEWAY_HOST");
    }
    ...
    async fn env_override_host_fallback() {
        ...
        crate::util::remove_env_var("SEN_GATEWAY_HOST");
        crate::util::set_env_var("HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        crate::util::remove_env_var("HOST");
    }
    ...
    async fn env_override_require_pairing() {
        ...
        crate::util::set_env_var("SEN_REQUIRE_PAIRING", "false");
        config.apply_env_overrides();
        assert!(!config.gateway.require_pairing);

        crate::util::set_env_var("SEN_REQUIRE_PAIRING", "true");
        config.apply_env_overrides();
        assert!(config.gateway.require_pairing);

        crate::util::remove_env_var("SEN_REQUIRE_PAIRING");
    }
    ...
    async fn env_override_temperature() {
        ...
        crate::util::set_env_var("SEN_TEMPERATURE", "0.5");
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        crate::util::remove_env_var("SEN_TEMPERATURE");
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        crate::util::set_env_var("SEN_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
    ...
    async fn env_override_reasoning_enabled() {
        ...
        crate::util::set_env_var("SEN_REASONING_ENABLED", "false");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        crate::util::set_env_var("SEN_REASONING_ENABLED", "true");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        crate::util::remove_env_var("SEN_REASONING_ENABLED");
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        crate::util::set_env_var("SEN_REASONING_ENABLED", "maybe");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        crate::util::set_env_var("SEN_REASONING_EFFORT", "HIGH");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        crate::util::remove_env_var("SEN_REASONING_EFFORT");
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        crate::util::set_env_var("SEN_CODEX_REASONING_EFFORT", "minimal");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        crate::util::remove_env_var("SEN_CODEX_REASONING_EFFORT");
    }
    ...
    async fn env_override_invalid_port_ignored() {
        ...
        crate::util::set_env_var("PORT", "not_a_number");
        config.apply_env_overrides();
    ...
    async fn env_override_web_search_config() {
        ...
        crate::util::set_env_var("WEB_SEARCH_ENABLED", "false");
        crate::util::set_env_var("WEB_SEARCH_PROVIDER", "brave");
        crate::util::set_env_var("WEB_SEARCH_MAX_RESULTS", "7");
        crate::util::set_env_var("WEB_SEARCH_TIMEOUT_SECS", "20");
        crate::util::set_env_var("BRAVE_API_KEY", "brave-test-key");
        ...
        crate::util::remove_env_var("WEB_SEARCH_ENABLED");
        crate::util::remove_env_var("WEB_SEARCH_PROVIDER");
        crate::util::remove_env_var("WEB_SEARCH_MAX_RESULTS");
        crate::util::remove_env_var("WEB_SEARCH_TIMEOUT_SECS");
        crate::util::remove_env_var("BRAVE_API_KEY");
    }
    ...
    async fn env_override_web_search_invalid_values_ignored() {
        ...
        crate::util::set_env_var("WEB_SEARCH_MAX_RESULTS", "99");
        crate::util::set_env_var("WEB_SEARCH_TIMEOUT_SECS", "0");
    ...
    async fn env_override_storage_provider_config() {
        ...
        crate::util::set_env_var("SEN_STORAGE_PROVIDER", "qdrant");
        crate::util::set_env_var("SEN_STORAGE_DB_URL", "http://localhost:6333");
        crate::util::set_env_var("SEN_STORAGE_CONNECT_TIMEOUT_SECS", "15");
        ...
        crate::util::remove_env_var("SEN_STORAGE_PROVIDER");
        crate::util::remove_env_var("SEN_STORAGE_DB_URL");
        crate::util::remove_env_var("SEN_STORAGE_CONNECT_TIMEOUT_SECS");
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        crate::util::set_env_var("SEN_PROXY_ENABLED", "true");
        crate::util::set_env_var("SEN_HTTP_PROXY", "http://127.0.0.1:7890");
        crate::util::set_env_var(
            "SEN_PROXY_SERVICES",
            "provider.openai, tool.http_request",
        );
        crate::util::set_env_var("SEN_PROXY_SCOPE", "services");
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        let mut config = Config::default();
        crate::util::set_env_var("SEN_PROXY_ENABLED", "true");
        crate::util::set_env_var("SEN_PROXY_SCOPE", "environment");
        crate::util::set_env_var("SEN_HTTP_PROXY", "http://127.0.0.1:7890");
        crate::util::set_env_var("SEN_HTTPS_PROXY", "http://127.0.0.1:7891");
        crate::util::set_env_var("SEN_NO_PROXY", "localhost,127.0.0.1");
...
[security.estop]
enabled = true
state_file = "~/.senweavercoding/estop-state.json"
require_otp_to_resume = true
"#,
        );
...
workspace_dir = "/sen-data/workspace"
config_path = "/sen-data/.senweavercoding/config.toml"
api_key = ""
default_provider = "openrouter"
