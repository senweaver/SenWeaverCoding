...
///
/// Checks `SEN_LOCALE`, then `LANG`, then `LC_ALL`.
...
"#,
    ...
    fn detect_locale_from_env() {
        ...
        let saved = std::env::var("SEN_LOCALE").ok();
        let saved_lang = std::env::var("LANG").ok();

        crate::util::set_env_var("SEN_LOCALE", "ja-JP");
        assert_eq!(detect_locale(), "ja-JP");

        crate::util::remove_env_var("SEN_LOCALE");
        crate::util::set_env_var("LANG", "zh_CN.UTF-8");
        assert_eq!(detect_locale(), "zh-CN");

        // Restore.
        match saved {
            Some(v) => crate::util::set_env_var("SEN_LOCALE", v),
            None => crate::util::remove_env_var("SEN_LOCALE"),
        }
        match saved_lang {
            Some(v) => crate::util::set_env_var("LANG", v),
            None => crate::util::remove_env_var("LANG"),
        }
    }
