...
// NOTE: Using parking_lot::Mutex for in-memory state. For high-concurrency scenarios,
// consider flume or tokio's async Mutex for better async integration.
#[derive(Debug, Clone)]
...
impl PairingGuard {
    ...
    pub async fn try_pair(&self, code: &str, client_id: &str) -> Result<Option<String>, u64> {
        // Use spawn_blocking for CPU-bound SHA256 hashing in try_pair_blocking.
        // For pure async workloads, prefer direct await without task spawning.
        let this = self.clone();
