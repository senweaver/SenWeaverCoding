...
impl SecurityPolicy {
    ...
    #[cfg(test)]
    pub fn allow_all() -> SecurityPolicy {
        SecurityPolicy {
    ...
    pub fn validate_command_execution(
        ...
        //
        // SECURITY WARNING: This configuration disables ALL command restrictions.
        // Use only in trusted environments or for debugging purposes.
    ...
    /// Also checks SEN_READ_ONLY and SEN_DRY_RUN environment variables from CLI
    pub fn can_act(&self) -> bool {
        // CLI --read-only flag takes precedence
        if std::env::var("SEN_READ_ONLY").as_deref() == Ok("1") {
            return false;
        }
        // CLI --dry-run mode simulates actions without actual changes
        if std::env::var("SEN_DRY_RUN").as_deref() == Ok("1") {
            return true; // Dry-run still allows "acting" but tools should check SEN_DRY_RUN
        }
        self.autonomy != AutonomyLevel::ReadOnly
    }

    /// Check if running in dry-run mode (CLI --dry-run flag)
...
mod tests {
    ...
    fn absolute_path_inside_workspace_allowed_when_workspace_only() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/user/.senweavercoding/workspace"),
            workspace_only: true,
            ..SecurityPolicy::default()
        };
        // Absolute path inside workspace should be allowed
        assert!(p.is_path_allowed("/home/user/.senweavercoding/workspace/images/example.png"));
        assert!(p.is_path_allowed("/home/user/.senweavercoding/workspace/file.txt"));
    ...
    fn absolute_path_in_allowed_root_permitted_when_workspace_only() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/user/.senweavercoding/workspace"),
            workspace_only: true,
            allowed_roots: vec![PathBuf::from("/home/user/.senweavercoding/shared")],
            ..SecurityPolicy::default()
        };
        // Path in allowed root should be permitted
        assert!(p.is_path_allowed("/home/user/.senweavercoding/shared/data.txt"));
    ...
    fn workspace_only_false_allows_resolved_outside_workspace() {
        let workspace = std::env::temp_dir().join("sen_test_ws_only_false");
    ...
    fn workspace_only_true_blocks_resolved_outside_workspace() {
        let workspace = std::env::temp_dir().join("sen_test_ws_only_true");
        let _ = std::fs::create_dir_all(&workspace);
        let canonical_workspace = workspace
            .canonicalize()
            .unwrap_or_else(|_| workspace.clone());
    ...
    fn resolved_path_blocks_outside_workspace() {
        let workspace = std::env::temp_dir().join("sen_test_resolved_path");
        let _ = std::fs::create_dir_all(&workspace);
        ...
        let canonical_temp = std::env::temp_dir()
            .canonicalize()
            .unwrap_or_else(|_| std::env::temp_dir());
        let outside = canonical_temp.join("outside_workspace_sen");
        assert!(
    ...
    fn resolved_path_blocks_root_escape() {
        let policy = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/sen_user/project"),
            ..SecurityPolicy::default()
        };
    ...
    fn resolved_path_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("sen_test_symlink_escape");
    ...
    fn allowed_roots_permits_paths_outside_workspace() {
        ...
        let root = std::env::temp_dir().join("sen_test_allowed_roots");
        let workspace = root.join("workspace");
    ...
    fn resolve_tool_path_normalizes_workspace_prefixed_relative_paths() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/sen-data/workspace"),
            ..SecurityPolicy::default()
        };
        let resolved = p.resolve_tool_path("sen-data/workspace/scripts/daily.py");
        assert_eq!(
            resolved,
            PathBuf::from("/sen-data/workspace/scripts/daily.py")
        );
    }
    ...
    fn runtime_config_paths_are_protected() {
        let workspace = PathBuf::from("/tmp/sen-profile/workspace");
        let policy = SecurityPolicy {
            workspace_dir: workspace.clone(),
            ..SecurityPolicy::default()
        };
