...
fn generate_policy(workspace: &Path) -> String {
    ...
    // SECURITY WARNING: The `(literal "/")` entry allows reading the entire root
    // filesystem. This is intentionally permissive for development but should be
    // restricted to specific paths in production deployments (e.g., /usr, /bin).
    format!(
        r#"(version 1)
