...
impl AuditLogger {
    ...
    ///
    /// If `config.sign_events` is true, requires `SEN_AUDIT_SIGNING_KEY` env var
...
/// - If a record has a `signature` field and `SEN_AUDIT_SIGNING_KEY` is available,
...
mod tests {
    ...
    /// Mutex to serialize tests that read/write SEN_AUDIT_SIGNING_KEY env var.
    ...
    fn audit_event_with_actor() {
        ...
        assert_eq!(actor.username, Some("@sen_user".to_string()));
    }
    ...
    fn signature_present_when_sign_events_enabled() -> Result<()> {
        ...
        let old_key = std::env::var("SEN_AUDIT_SIGNING_KEY").ok();
        defer! {
            if let Some(key) = old_key {
                crate::util::set_env_var("SEN_AUDIT_SIGNING_KEY", key);
            } else {
                crate::util::remove_env_var("SEN_AUDIT_SIGNING_KEY");
            }
        }

        let tmp = TempDir::new()?;
        let test_key = "a".repeat(64); // 64 hex chars = 32 bytes
        crate::util::set_env_var("SEN_AUDIT_SIGNING_KEY", &test_key);
    ...
    fn constructor_fails_if_sign_events_but_no_key() -> Result<()> {
        ...
        if let Err(e) = result {
            ...
            assert!(
                err_msg.contains("SEN_AUDIT_SIGNING_KEY not set"),
                "error: {}",
                err_msg
            );
        }
    ...
    fn constructor_fails_if_signing_key_invalid_hex() -> Result<()> {
        ...
        crate::util::set_env_var("SEN_AUDIT_SIGNING_KEY", "not-valid-hex");
    ...
    fn constructor_fails_if_signing_key_wrong_length() -> Result<()> {
        ...
        crate::util::set_env_var("SEN_AUDIT_SIGNING_KEY", &short_key);
        let tmp = TempDir::new()?;
        let config = AuditConfig {
            enabled: true,
            sign_events: true,
