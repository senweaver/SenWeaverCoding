...
impl PromptGuard {
    ...
    pub fn scan(&self, content: &str) -> GuardResult {
        ...
        // Each category contributes up to 1.0, so max possible is 6.0 (one per category).
        // We use the sum divided by number of categories to reflect cumulative risk.
        // Using min() ensures the final score is always clamped to [0.0, 1.0].
        ...
        if detected_patterns.is_empty() {
            ...
            // Use max_category_score for threshold decisions (individual category risk)
            // Use normalized_score for display (cumulative risk indicator)
            match self.action {
