...
use super::web_search::engine::{ApiKeys, SearchCategory, SearchContext, SearchHit, TimeRange};
use super::web_search::ranker::{
    academic_merge, filter_by_relevance, merge_and_dedup, render_results_markdown,
};
use super::web_search::routing::global_registry;
...
impl WebSearchTool {
    pub fn new(
        ...
        Self {
            ...
            max_results: max_results
                .max(WEB_SEARCH_DEFAULT_RESULTS)
                .min(WEB_SEARCH_RESULT_HARD_CAP),
    ...
    fn build_context(&self, args: &serde_json::Value) -> anyhow::Result<(SearchContext, bool, Option<String>)> {
        ...
        let ctx = SearchContext {
            ...
            user_agent: super::web_search::engine::default_user_agent(),
            extra,
        };
...
fn compute_per_engine_limit(global_limit: usize, fanout: usize) -> usize {
    ...
    let base = (global_limit + fanout - 1) / fanout;
    base.saturating_add(2).max(WEB_SEARCH_PER_ENGINE_MIN)
}

async fn run_engine_with_retry(
    engine: &Arc<dyn super::web_search::engine::SearchEngine>,
    ctx: &SearchContext,
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if !merged.is_empty() && !diag.successful_engines.is_empty() {
            output.push_str(&format!(
                "Sources: {} engine(s) returned {} aggregated result(s) — {}\n\n",
...
impl WebSearchTool {
    ...
    async fn search_hits_internal(
        ...
        let fanout = chain.len().min(WEB_SEARCH_MAX_FANOUT).max(1);
        ...
        for engine in primary_engines.iter().cloned() {
        ...
        if current_total < WEB_SEARCH_MIN_RESULTS_TARGET && chain.len() > fanout {
            ...
            if !overflow_engines.is_empty() {
                ...
                for engine in overflow_engines.iter().cloned() {
