...
//! Wraps a discovered MCP tool as a sen [`Tool`] so it is dispatched
...
/// A sen [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // SenWeaverCoding's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // SenWeaverCoding's security model injects `approved: bool` into built-in tool args.
