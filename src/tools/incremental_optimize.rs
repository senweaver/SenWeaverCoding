...
/// A recorded change in the optimization track.
...
/// A checkpoint recording the state before changes.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Shared state for incremental optimization tracking.
#[derive(Debug, Clone)]
...
/// The Incremental Optimization tool.
...
impl IncrementalOptimizeTool {
    pub fn new(state: OptimizationStateHandle, workspace_dir: std::path::PathBuf) -> Self {
    ...
    fn generate_suggestions_for_change(
        ...
        // Heuristic: detect performance issues
        if change.change_type == ChangeType::Modified || change.change_type == ChangeType::Added {
            ...
            if content.contains("for ") && content.contains("for ") {
                ...
                if nested_for_count >= 2 {
                    suggestions.push(OptimizationSuggestion {
                        ...
                        rationale: "Nested loops are a common source of O(n^2) performance issues.".to_string(),
            ...
            // `.clone()` chain heuristic (Rust-specific)
            ...
            // Long function heuristic (>100 lines)
            let code_lines: Vec<&&str> = lines
                .iter()
                .filter(|l| !l.trim().is_empty() && !l.trim().starts_with("//") && !l.trim().starts_with("/*") && !l.trim().starts_with("*"))
                .collect();
            if code_lines.len() > 100 {
                suggestions.push(OptimizationSuggestion {
                    ...
                    description: format!("Function/file appears to have {} non-comment lines. Consider splitting.", code_lines.len()),
    ...
    fn format_status(&self, state: &OptimizationState) -> String {
        ...
        lines.push(format!(
            ...
            state.stats.suggestions_applied
        ));
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        match action {
            ...
            "suggest" => {
                ...
                let mut new_suggestions = Vec::new();
                {
                    ...
                    for change in &changes {
                        ...
                        let file_path = self.workspace_dir.join(&change.file);
                        if let Ok(content) = std::fs::read_to_string(&file_path) {
                            let file_suggestions = self.generate_suggestions_for_change(change, &content);
                            for sug in file_suggestions {
                                // Avoid duplicates
                                if !state.suggestions.iter().any(|s| s.description == sug.description) {
            ...
            "verify" => {
                ...
                // Run a verification command if provided
                    ...
                    match result {
                        ...
                        Err(e) => {
                            return Ok(ToolResult {
                                success: false,
                                output: String::new(),
                                error: Some(format!("Failed to run verification command '{cmd}': {e}")),
                            });
                        }
                    }
                }
            ...
            "report" => {
                ...
                if state.checkpoints.is_empty() {
                    ...
                    for cp in &state.checkpoints {
                        report.push_str(&format!(
                            "- `{}` (ts={}): {}\n  Files: {}\n",
                            cp.id, cp.created_at, cp.description, cp.files.len()
                        ));
                    }
                    report.push('\n');
                }
...
impl IncrementalOptimizeTool {
    fn list_code_files(&self) -> anyhow::Result<Vec<String>> {
        let extensions = ["rs", "ts", "tsx", "js", "jsx", "py", "go", "java", "toml", "yaml", "yml"];
        let mut files = Vec::new();

        fn walk(dir: &std::path::Path, extensions: &[&str], workspace: &std::path::Path, files: &mut Vec<String>) -> anyhow::Result<()> {
            for entry in std::fs::read_dir(dir)? {
                let entry = entry?;
                let path = entry.path();
                if path.is_dir() {
                    if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                        if !name.starts_with('.') && name != "target" && name != "node_modules" && name != "__pycache__" && name != "dist" && name != "build" {
        ...
        walk(&self.workspace_dir, &extensions, &self.workspace_dir, &mut files)?;
        Ok(files)
    }
}
...
mod tests {
    ...
    fn make_tool() -> IncrementalOptimizeTool {
        IncrementalOptimizeTool::new(
            Arc::new(RwLock::new(OptimizationState::default())),
            std::path::PathBuf::from("."),
        )
    }

    #[tokio::test]
    async fn checkpoint_creates_checkpoint() {
        let tool = make_tool();
        let result = tool
            .execute(json!({"action": "checkpoint", "description": "test checkpoint"}))
            .await
            .unwrap();
        assert!(result.success);
        assert!(result.output.contains("checkpoint"));
    }

    #[tokio::test]
    async fn track_records_change() {
        let tool = make_tool();
        let result = tool
            .execute(json!({
                "action": "track",
            ...
            .unwrap();
        assert!(result.success);
        assert!(result.output.contains("Change [1] tracked"));
    }

    #[tokio::test]
    async fn status_shows_current_state() {
        let tool = make_tool();
        let result = tool.execute(json!({"action": "status"})).await.unwrap();
        assert!(result.success);
        assert!(result.output.contains("Incremental Optimization Status"));
    }

    #[tokio::test]
    async fn report_generates_markdown() {
        let tool = make_tool();
        let result = tool
            .execute(json!({"action": "report", "description": "Test Report"}))
            .await
            .unwrap();
        assert!(result.success);
        assert!(result.output.contains("Test Report"));
    ...
    #[tokio::test]
    async fn verify_change_marks_verified() {
        let tool = make_tool();
        // First track a change
        tool.execute(json!({
            "action": "track",
            "file": "src/main.rs",
            "change_type": "added",
        ...
        // Then verify it
        let result = tool
            .execute(json!({"action": "verify", "change_id": 1}))
            .await
            .unwrap();
        assert!(result.success);
    ...
    #[tokio::test]
    async fn track_requires_file() {
        let tool = make_tool();
        let result = tool
            .execute(json!({"action": "track", "change_type": "modified"}))
            .await;
        // execute must fail when 'file' is missing
        assert!(result.is_err(), "execute should error when 'file' is missing, got {:?}", result);
    }

    #[test]
    fn suggestion_generation_detects_nested_loops() {
        let tool = make_tool();
        let change = TrackedChange {
            file: "test.rs".to_string(),
            change_type: ChangeType::Modified,
            summary: "added nested loops".to_string(),
            lines_added: 10,
            lines_removed: 0,
            verified: false,
            verified_at: None,
        };
        let content = "for i in 0..n {\n    for j in 0..m {\n        println!(\"{}\", i + j);\n    }\n}";
        let suggestions = tool.generate_suggestions_for_change(&change, content);
        assert!(!suggestions.is_empty());
        assert!(suggestions.iter().any(|s| matches!(s.category, SuggestionCategory::Performance)));
    }

    #[test]
    fn suggestion_generation_detects_unwrap() {
        let tool = make_tool();
        let change = TrackedChange {
            file: "test.rs".to_string(),
            change_type: ChangeType::Added,
            summary: "added unwrap".to_string(),
            lines_added: 5,
            lines_removed: 0,
            verified: false,
            verified_at: None,
        ...
        let suggestions = tool.generate_suggestions_for_change(&change, content);
        assert!(suggestions.iter().any(|s| matches!(s.category, SuggestionCategory::ErrorHandling)));
    }

    #[test]
    fn suggestion_generation_detects_secrets() {
        let tool = make_tool();
        let change = TrackedChange {
            file: "config.rs".to_string(),
            change_type: ChangeType::Added,
            summary: "added config".to_string(),
            lines_added: 3,
            lines_removed: 0,
            verified: false,
            verified_at: None,
        ...
        let suggestions = tool.generate_suggestions_for_change(&change, content);
        assert!(suggestions.iter().any(|s| matches!(s.category, SuggestionCategory::Security)));
    }

    #[test]
    fn suggestion_generation_detects_clone_chain() {
        let tool = make_tool();
        let change = TrackedChange {
            file: "test.rs".to_string(),
            change_type: ChangeType::Modified,
            summary: "chained clones".to_string(),
            lines_added: 2,
            lines_removed: 0,
            verified: false,
            verified_at: None,
        };
        let content = "let x = s.clone().clone().to_string();";
        let suggestions = tool.generate_suggestions_for_change(&change, content);
        assert!(suggestions.iter().any(|s| s.description.contains("Redundant clone")));
    }

    #[test]
    fn tool_name() {
        assert_eq!(make_tool().name(), "incremental_optimize");
    }
}
