...
impl DelegateTool {
    ...
    async fn execute_sync(
        ...
        // Track delegate sub-agent in multi-agent runtime
        crate::agent::runtime_hooks::track_delegate_spawn(
            agent_name,
        ...
        match result {
            Ok(response) => {
                ...
                // Track delegate completion in multi-agent runtime
                crate::agent::runtime_hooks::track_delegate_complete(agent_name, true);
            ...
            Err(e) => {
                // Track delegate failure in multi-agent runtime
                crate::agent::runtime_hooks::track_delegate_complete(agent_name, false);
                Ok(ToolResult {
                    success: false,
                    output: String::new(),
...
mod tests {
    ...
    async fn cancel_task_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "sen_delegate_traversal_cancel_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
