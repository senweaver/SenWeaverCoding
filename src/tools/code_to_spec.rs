...
fn detect_language(path: &Path) -> String {
    match path.extension().and_then(|e| e.to_str()) {
        ...
        Some("js" | "jsx" | "mjs") => "javascript".to_string(),
    ...
    while i < lines.len() {
        ...
        if line.starts_with("pub struct ") || line.starts_with("struct ") {
            if let Some(name_end) = find_first_of(line, &['(', '{', ';']) {
                let name_start = if line.starts_with("pub struct ") {
                    11
                } else {
                    7
                };
        ...
        if line.starts_with("pub trait ") || line.starts_with("trait ") {
            if let Some(name_end) = find_first_of(line, &['{', ':']) {
                let name_start = if line.starts_with("pub trait ") {
                    10
                } else {
                    6
                };
...
fn extract_ts_structures(content: &str, file_path: &str) -> Vec<CodeStructure> {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("export class ") || line.starts_with("class ") {
            let prefix_len = if line.starts_with("export class ") {
                13
            } else {
                6
            };
        ...
        if line.starts_with("export interface ") || line.starts_with("interface ") {
            let prefix_len = if line.starts_with("export interface ") {
                17
            } else {
                10
            };
            if let Some(brace) = line[prefix_len..]
                .find('{')
                .or_else(|| line[prefix_len..].find(' '))
            {
...
fn extract_python_structures(content: &str, file_path: &str) -> Vec<CodeStructure> {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("class ") || line.starts_with("async class ") {
            let prefix_len = if line.starts_with("async class ") {
                12
            } else {
                6
            };
        ...
        if line.starts_with("def ") || line.starts_with("async def ") {
            ...
            let paren_end = content_part[name_end..]
                .find(')')
                .map(|p| name_end + p + 1)
                .unwrap_or(line.len());
...
fn build_spec_markdown(
    ...
    md.push_str(&format!(
        "> Auto-generated by `code_to_spec` tool. Last updated: {}\n\n",
        chrono_lite_now()
    ));
...
fn compare_spec_with_code(
    ...
    for heading in &spec_headings {
        ...
        let found = files
            .values()
            .any(|c| c.contains(&format!(" {}", name)) || c.contains(&format!(" {}", name)));
    ...
    if gaps.is_empty() {
        ...
        format!(
            "### Gaps Detected\n\n{}\n\n> This is a heuristic scan. Manual review recommended.",
            gaps.join("\n")
        )
    }
}
...
impl CodeToSpecTool {
    pub fn new(workspace_root: Arc<RwLock<std::path::PathBuf>>) -> Self {
        Self {
            workspace_root,
            security: None,
        }
    ...
    fn resolve_path(&self, path: &str) -> std::path::PathBuf {
        ...
        if p.is_absolute() {
            ...
            self.workspace_snapshot().join(path)
        }
    }
    ...
    fn run_analysis(&self, files: &HashMap<String, String>) -> HashMap<String, AnalysisResult> {
    ...
    fn summarize_state(
        ...
        let language_counts: HashMap<&str, usize> =
            analysis.values().fold(HashMap::new(), |mut acc, r| {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        match action {
            ...
            "generate" => {
                ...
                let full_path = self.workspace_snapshot().join(&output_path);
                ...
                if let Some(ref security) = self.security {
                    ...
                    if !security.is_path_allowed(&output_path) {
                        return Ok(ToolResult {
                            success: false,
                            output: String::new(),
                            error: Some(format!(
                                "Path not allowed by security policy: {output_path}"
                            )),
                        });
                    }
                    if security.is_runtime_config_path(&full_path) {
                        return Ok(ToolResult {
                            success: false,
                            output: String::new(),
                            error: Some(format!(
                                "Refusing to overwrite runtime config file: {output_path}"
                            )),
                        });
                    }
