...
/// Maximum output size in bytes (1MB) for raw capture.
const MAX_OUTPUT_BYTES: usize = 1_048_576;
/// Default output cap returned to the LLM (32KB) to keep context lean.
...
impl Tool for ShellTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Per-call timeout override via timeout_ms parameter
        ...
        match result {
            Ok(Ok((status, mut stdout, mut stderr))) => {
                // Truncate raw output to prevent OOM
                ...
                // Apply LLM-friendly output cap (32KB) to keep context lean
...
mod tests {
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            match &self.original {
                Some(val) => crate::util::set_env_var(self.key, val),
                None => crate::util::remove_env_var(self.key),
            }
        }
    }
    ...
    async fn shell_does_not_leak_api_key() {
        ...
        let _g2 = EnvGuard::set("SEN_API_KEY", "sk-test-secret-67890");
        ...
        assert!(
            ...
            "SEN_API_KEY leaked to shell command output"
        );
    }
    ...
    async fn shell_allows_configured_env_passthrough() {
        let _guard = EnvGuard::set("SEN_TEST_PASSTHROUGH", "db://unit-test");
        let tool = ShellTool::new(
            test_security_with_env_passthrough(&["SEN_TEST_PASSTHROUGH"]),
            test_runtime(),
        );
        ...
        assert!(
            result
                .output
                .contains("SEN_TEST_PASSTHROUGH=db://unit-test")
        );
    }
    ...
    async fn shell_requires_approval_for_medium_risk_command() {
        ...
        let denied = tool
            .execute(json!({"command": "touch sen_shell_approval_test"}))
            .await
        ...
        let allowed = tool
            .execute(json!({
                "command": "touch sen_shell_approval_test",
                "approved": true
            }))
            .await
        ...
        let _ =
            tokio::fs::remove_file(std::env::temp_dir().join("sen_shell_approval_test")).await;
    }
