...
/// Windows PowerShell execution tool with security validation.
///
/// Distinct from the generic `ShellTool` — provides PowerShell-specific
/// command semantics, Constrained Language Mode awareness, and
/// Windows-specific path safety.
...
mod tests {
    ...
    #[test]
    fn name_matches() {
        assert_eq!(PowerShellTool::new(test_security()).name(), "powershell");
    }

    #[test]
    fn schema_has_command() {
        let tool = PowerShellTool::new(test_security());
    ...
    async fn blocks_readonly() {
        ...
        let tool = PowerShellTool::new(sec);
        let result = tool
