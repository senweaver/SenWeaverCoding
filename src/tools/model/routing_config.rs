...
use super::super::traits::{Tool, ToolResult, json_verbose_flag, render_json_output};
...
impl ModelRoutingConfigTool {
    ...
    fn handle_get(&self) -> anyhow::Result<ToolResult> {
        ...
        Ok(ToolResult {
            success: true,
            output: render_json_output(&Self::snapshot(&cfg), false),
            error: None,
        })
    }
    ...
    async fn probe_model(&self, provider_name: &str, model: &str) -> anyhow::Result<()> {
        ...
        let provider = match providers::create_provider_with_url_async(
