...
mod tests {
    ...
    fn test_security() -> Arc<SecurityPolicy> {
        ...
        p.workspace_dir = std::env::temp_dir().join("fs_ops_test");
        Arc::new(p)
    }
    ...
    fn tool_names() {
        ...
        assert_eq!(CopyPathTool::new(sec.clone()).name(), "copy_path");
        assert_eq!(MovePathTool::new(sec.clone()).name(), "move_path");
        assert_eq!(DeletePathTool::new(sec.clone()).name(), "delete_path");
        assert_eq!(CreateDirectoryTool::new(sec).name(), "create_directory");
    }
    ...
    fn schemas_valid() {
        let sec = test_security();
        assert_eq!(
            CopyPathTool::new(sec.clone()).parameters_schema()["type"],
            "object"
        );
        assert_eq!(
            CreateDirectoryTool::new(sec).parameters_schema()["type"],
            "object"
        );
    }
}
