...
pub static TOOL_TIERS: LazyLock<HashMap<&'static str, ToolTierEntry>> = LazyLock::new(|| {
    ...
    let enhanced_entries: &[(&str, ToolRiskLevel, &str)] = &[
        ...
        ("coding_session_cli", MODERATE, "Drive an external coding-session CLI"),
        ("coding_session_runner", MODERATE, "Run a sub-task via the external coding-session runner"),
        ("inline_coding_cli", MODERATE, "Drive an inline (quiet-mode) coding CLI"),
        ("shell_coding_cli", MODERATE, "Drive a shell-enabled coding CLI"),
