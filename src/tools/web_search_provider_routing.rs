...
mod tests {
    ...
    #[test]
    fn resolve_aliases_to_exa() {
        let exa_aliases = ["exa", "exa-search", "exa_search"];
        for alias in exa_aliases {
            let resolved = resolve_web_search_provider(alias);
            assert_eq!(resolved.route, WebSearchProviderRoute::Exa);
            assert_eq!(resolved.canonical_provider, EXA_PROVIDER);
            assert!(!resolved.used_fallback);
        }
    }
