...
impl Tool for ExitPlanModeTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        *self.pending_plan.write() = Some(plan_text.to_string());
        ...
        let (path_disp, wrapped_md, write_note) = match workspace
            .and_then(|w| write_plan_file_under(&w, plan_text))
        {
            Ok((p, body)) => {
...
fn quality_check(trimmed: &str) -> Result<(), String> {
    if trimmed.is_empty() {
        return Err(
            "exit_plan_mode requires a non-empty `plan_content` argument. Draft the full \
             Cursor-style markdown (frontmatter + ## sections + ≥3 todos with file refs) and \
             call exit_plan_mode again.".to_string(),
        );
    }
    ...
    if section_count < 2 {
        missing.push(format!(
            "missing structural sections ({section_count} `## ` headings; \
             need at least `## 工作量摸底` + `## Track 1 — …` + `## 验收`)"
        ));
    }
    if !has_file_refs {
        missing.push(
    ...
    if !has_code_fence {
        missing.push(
            "no fenced code block (the `## 验收` section MUST contain at least \
    ...
    Err(format!(
        "exit_plan_mode REJECTED — the supplied `plan_content` is too thin to be \
         a real plan.  Plan mode demands a Cursor-style document (see \
         `plan-create-completed.png`) — what's missing:\n{bullets}\n\n\
         ...
         2. DECOMPOSE the task into 5-10 ordered todos by file group, NOT a \
            ...
            `Glob-replace all .go imports: \"old/...\" → \"new/...\" (149 files)`, \
            `Update Dockerfile: ldflags path + binary name`).\n\
         3. WRITE the full Cursor-style markdown body with `## 工作量摸底` \
            (linking each affected file via `[path](path)`), one or more \
            `## Track N — …` sections, and `## 验收` with verification \
