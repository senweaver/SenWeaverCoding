...
mod tests {
    ...
    fn ensure_browser_env_sets_chromium_flags() {
        ...
        crate::util::remove_env_var("CHROMIUM_FLAGS");

        let mut cmd = Command::new("true");
        ensure_browser_env(&mut cmd);

        if let Some(val) = original {
            crate::util::set_env_var("CHROMIUM_FLAGS", &val);
        }
    }
    ...
    fn is_service_environment_detects_invocation_id() {
        ...
        crate::util::set_env_var("INVOCATION_ID", "test-unit-id");

        assert!(is_service_environment());

        if let Some(val) = original {
            crate::util::set_env_var("INVOCATION_ID", &val);
        } else {
            crate::util::remove_env_var("INVOCATION_ID");
        }
    }
    ...
    fn is_service_environment_detects_journal_stream() {
        ...
        crate::util::set_env_var("JOURNAL_STREAM", "8:12345");

        assert!(is_service_environment());

        if let Some(val) = original {
            crate::util::set_env_var("JOURNAL_STREAM", &val);
        } else {
            crate::util::remove_env_var("JOURNAL_STREAM");
        }
    }
