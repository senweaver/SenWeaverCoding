...
impl SearchHit {
    ...
    /// Stable identity key for academic search results across providers.
    /// Priority: DOI (extra.doi or doi.org URL) > arXiv ID (arxiv.org/abs path)
    /// > normalized title. Returns `None` when no stable identity exists,
    /// signalling the merger to leave the hit untouched.
