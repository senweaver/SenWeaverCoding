...
static BING_BLOCK_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<li[^>]*class="[^"]*b_algo[^"]*"[^>]*>([\s\S]*?)</li>"#).expect("bing block regex")
});
static BING_LINK_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<h2[^>]*>[\s\S]*?<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)</a>[\s\S]*?</h2>"#)
        .expect("bing link regex")
});
static BING_SNIPPET_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<(?:p|div)[^>]*class="[^"]*(?:b_lineclamp|b_caption|b_snippet|b_paractl)[^"]*"[^>]*>([\s\S]*?)</(?:p|div)>"#)
        .expect("bing snippet regex")
});
static BING_SOURCE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<cite[^>]*>([\s\S]*?)</cite>"#).expect("bing source regex")
});
