...
static YAHOO_NEWS_ITEM_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<li[^>]*class="[^"]*ov[^"]*"[^>]*>([\s\S]*?)</li>"#)
        .expect("yahoo news item regex")
});
static YAHOO_NEWS_LINK_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<h4[^>]*>\s*<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)</a>"#)
        .expect("yahoo news link regex")
});
static YAHOO_NEWS_DESC_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<p[^>]*class="[^"]*s-desc[^"]*"[^>]*>([\s\S]*?)</p>"#)
        .expect("yahoo news desc regex")
});
static YAHOO_NEWS_META_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<span[^>]*class="[^"]*s-source[^"]*"[^>]*>([\s\S]*?)</span>"#)
        .expect("yahoo news meta regex")
});
