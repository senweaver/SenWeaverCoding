...
static WX_LI_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<li[^>]*(?:id="sogou_vr_[\w-]+"|class="[^"]*(?:results|news-list)[^"]*"[^>]*)?[^>]*>([\s\S]*?)</li>"#)
        .expect("weixin li regex")
});
static WX_NEWSLIST_LI_FALLBACK_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<ul[^>]*class="[^"]*news-list[^"]*"[^>]*>([\s\S]*?)</ul>"#)
        .expect("weixin news-list scope regex")
});
static WX_LI_PLAIN_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<li[^>]*>([\s\S]*?)</li>"#).expect("weixin plain li regex")
});
static WX_LINK_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<h3[^>]*>[\s\S]*?<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)</a>"#)
        .expect("weixin link regex")
});
static WX_SNIPPET_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<p[^>]*class="[^"]*txt-info[^"]*"[^>]*>([\s\S]*?)</p>"#)
        .expect("weixin snippet regex")
});
static WX_ACCOUNT_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<a[^>]*class="[^"]*account[^"]*"[^>]*>([\s\S]*?)</a>"#)
        .expect("weixin account regex")
});
