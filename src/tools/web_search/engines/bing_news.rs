...
static BNEWS_CARD_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r#"<div[^>]*class="[^"]*news-card[^"]*"[^>]*data-url="([^"]+)"[^>]*>([\s\S]*?)</div>\s*</div>"#,
    )
    .expect("bing news card regex")
});
static BNEWS_TITLE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<a[^>]*class="title"[^>]*>([\s\S]*?)</a>"#).expect("bing news title regex")
});
static BNEWS_SNIPPET_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<div[^>]*class="snippet"[^>]*>([\s\S]*?)</div>"#)
        .expect("bing news snippet regex")
});
static BNEWS_SOURCE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<div[^>]*class="source[^"]*"[^>]*>([\s\S]*?)</div>"#)
        .expect("bing news source regex")
});
