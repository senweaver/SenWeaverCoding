...
static BAIDU_TITLE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<h3[^>]*class="[^"]*c-title[^"]*"[^>]*>[\s\S]{0,800}?<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)</a>"#)
        .expect("baidu title regex")
});
static BAIDU_ABSTRACT_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<(?:span|div|p)[^>]*class="[^"]*(?:c-abstract|content-right_[\w-]*|c-span-last|c-color-text|c-gap-top-small|cu-line-clamp[\w-]*|line-clamp[\w-]*|c-row[\w-]*|cos[a-zA-Z0-9_-]*)[^"]*"[^>]*>([\s\S]*?)</(?:span|div|p)>"#)
        .expect("baidu abstract regex")
});
static BAIDU_BLOCK_CLEANUP_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"<(?:script|style|a|button|input|select|noscript)\b[^>]*>[\s\S]*?</(?:script|style|a|button|input|select|noscript)>")
        .expect("baidu block cleanup regex")
});
static BAIDU_BLOCK_START_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<div[^>]+class="[^"]*\b(?:result|c-container)\b[^"]*""#)
        .expect("baidu block start regex")
});
static BAIDU_END_MARKER_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<div[^>]+id="(?:page|content_right|content_left_bottom)""#)
        .expect("baidu end marker regex")
});
