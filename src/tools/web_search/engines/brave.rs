...
static BRAVE_SNIPPET_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<div[^>]*class="[^"]*snippet[^"]*"[^>]*>([\s\S]*?)</div>\s*</div>"#)
        .expect("brave snippet block regex")
});
static BRAVE_LINK_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<a[^>]*class="[^"]*heading-serpresult[^"]*"[^>]*href="([^"]+)"[^>]*>([\s\S]*?)</a>"#)
        .expect("brave link regex")
});
static BRAVE_TITLE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<div[^>]*class="[^"]*title[^"]*"[^>]*>([\s\S]*?)</div>"#)
        .expect("brave title regex")
});
static BRAVE_DESC_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"<div[^>]*class="[^"]*snippet-description[^"]*"[^>]*>([\s\S]*?)</div>"#)
        .expect("brave snippet description regex")
});
