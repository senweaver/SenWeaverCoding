...
/// A resource advertised by an MCP server (from `resources/list` response).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Single resource body from `resources/read` (MCP wire uses camelCase `mimeType`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    #[test]
    fn list_resources_result_deserializes_mime_type() {
        let json = r#"{"resources":[{"uri":"file:///a","name":"a","mimeType":"text/plain"}]}"#;
        let result: ListResourcesResult = serde_json::from_str(json).unwrap();
        assert_eq!(result.resources.len(), 1);
        assert_eq!(result.resources[0].uri, "file:///a");
        assert_eq!(result.resources[0].mime_type.as_deref(), Some("text/plain"));
    }

    #[test]
    fn read_resource_result_deserializes_contents() {
        let json = r#"{"contents":[{"uri":"u","mimeType":"application/json","text":"{}"}]}"#;
