...
impl McpServer {
    ...
    /// List resources from this server (`resources/list`).
...
impl McpRegistry {
    ...
    /// List resources from connected MCP servers (`resources/list`).
    /// When `server_name` is `None`, merges lists from all servers (logs and skips on per-server failure).
    ...
    /// Read a resource via `resources/read` on the named server.
...
mod tests {
    ...
    async fn connect_nonexistent_command_fails_cleanly() {
        ...
        let config = McpServerConfig {
            ...
            command: "/usr/bin/this_binary_does_not_exist_sen_test".to_string(),
            args: vec![],
