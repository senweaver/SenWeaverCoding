...
impl Tool for MultiSearchTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if use_ddg {
            ...
            crate::runtime::spawn_supervised("tools.multi_search.duckduckgo", async move {
                let _ = tx.send(search_duckduckgo(&c, &q, mr).await);
            });
            handles.push(rx);
        }
        ...
        if use_brave {
            ...
            let mr = self.max_results;
            let (tx, rx) = tokio::sync::oneshot::channel();
            crate::runtime::spawn_supervised("tools.multi_search.brave", async move {
                let _ = tx.send(search_brave(&c, &q, &key, mr).await);
            });
            handles.push(rx);
        }
        ...
        if use_searxng {
            ...
            let mr = self.max_results;
            let (tx, rx) = tokio::sync::oneshot::channel();
            crate::runtime::spawn_supervised("tools.multi_search.searxng", async move {
                let _ = tx.send(search_searxng(&c, &q, &url, mr).await);
            });
            handles.push(rx);
        }
        ...
        if use_tavily {
            ...
            let mr = self.max_results;
            let (tx, rx) = tokio::sync::oneshot::channel();
            crate::runtime::spawn_supervised("tools.multi_search.tavily", async move {
                let _ = tx.send(search_tavily(&c, &q, &key, mr).await);
            });
            handles.push(rx);
        }
        ...
        if use_exa {
            ...
            let mr = self.max_results;
            let (tx, rx) = tokio::sync::oneshot::channel();
            crate::runtime::spawn_supervised("tools.multi_search.exa", async move {
                let _ = tx.send(search_exa(&c, &q, &key, mr).await);
            });
            handles.push(rx);
        }
        ...
        for handle in handles {
            match handle.await {
                ...
                Err(_) => {
                    errors.push("Search task panicked".to_string());
                }
            }
        }
