...
use crate::config::CodingSessionRunnerConfig;
...
pub struct CodingSessionHookEvent {
...
pub struct CodingSessionRunnerTool {
    security: Arc<SecurityPolicy>,
    config: CodingSessionRunnerConfig,

    gateway_url: String,
}

impl CodingSessionRunnerTool {
    pub fn new(
        security: Arc<SecurityPolicy>,
        config: CodingSessionRunnerConfig,
        gateway_url: String,
...
#[async_trait]
impl Tool for CodingSessionRunnerTool {
    fn name(&self) -> &str {
        "coding_session_runner"
    }

    fn description(&self) -> &str {
        "Spawn an external coding-session CLI task in a tmux session with live progress updates and SSH handoff. Returns immediately with session ID and attach command."
    }
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                "prompt": {
                    "type": "string",
                    "description": "The coding task to delegate to the external session CLI"
                },
                "working_directory": {
                    "type": "string",
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if let Err(error) = self
            .security
            .enforce_tool_operation(ToolOperation::Act, "coding_session_runner")
        {
            return Ok(ToolResult {
                success: false,
                output: String::new(),
        ...
        let hook_url = format!("{}/hooks/coding-session", self.gateway_url);
        ...
        cli_args.push("--hook-url".to_string());
        cli_args.push(hook_url.clone());
        ...
        use std::fmt::Write;
        let _ = write!(env_exports, "CODING_SESSION_ID={} ", &session_id);
        if let Some(ref ch) = slack_channel {
            let _ = write!(env_exports, "CODING_SESSION_SLACK_CHANNEL={} ", ch);
        }
        let _ = write!(env_exports, "CODING_SESSION_HOOK_URL={} ", &hook_url);
        ...
        match create_result {
            ...
            Err(e) => {
                return Ok(ToolResult {
                    ...
                    output: String::new(),
                    error: Some(format!(
                        "tmux not found or failed to execute: {e}. Install tmux to use coding_session_runner."
                    )),
                });
            }
            _ => {}
        ...
        let _ttl_cleanup = crate::runtime::spawn_supervised(
            format!("tools.coding_session_runner.ttl_cleanup.{}", session_name),
            async move {
                ...
                tracing::info!(
                    ...
                    "Coding session runner TTL expired, cleaned up"
                );
            },
        );
