...
use crate::config::CodingSessionCliConfig;
...
pub struct CodingSessionCliTool {
    security: Arc<SecurityPolicy>,
    config: CodingSessionCliConfig,
}

impl CodingSessionCliTool {
    pub fn new(security: Arc<SecurityPolicy>, config: CodingSessionCliConfig) -> Self {
        Self { security, config }
    }
}

#[async_trait]
impl Tool for CodingSessionCliTool {
    fn name(&self) -> &str {
        "coding_session_cli"
    }

    fn description(&self) -> &str {
        "Delegate a coding task to an external session-capable coding CLI (binary 'claude -p'). Supports file editing, bash execution, structured output, and multi-turn sessions. Use for complex coding work that benefits from a full external agent loop with persistent sessions."
    }
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                "prompt": {
                    "type": "string",
                    "description": "The coding task to delegate to the external coding session CLI"
                },
                "allowed_tools": {
                    "type": "array",
                    "items": { "type": "string" },
                ...
                "session_id": {
                    "type": "string",
                    "description": "Resume a previous coding session by its ID"
                },
                "json_schema": {
                    "type": "object",
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if let Err(error) = self
            .security
            .enforce_tool_operation(ToolOperation::Act, "coding_session_cli")
        {
            return Ok(ToolResult {
                success: false,
                output: String::new(),
        ...
        match result {
            ...
            Ok(Err(e)) => {
                ...
                {
                    "Coding session CLI ('claude') not found in PATH. Configure the binary via PATH or update [coding_session_cli] in config.".into()
                } else {
                    format!("Failed to execute coding session CLI: {e}")
                };
                Ok(ToolResult {
                    success: false,
                    output: String::new(),
            ...
            Err(_) => {
                ...
                Ok(ToolResult {
                    ...
                    output: String::new(),
                    error: Some(format!(
                        "Coding session CLI timed out after {}s and was killed",
