...
/// Bodies shorter than this are treated as JS-only pages that need Firecrawl/Jina.
const FIRECRAWL_MIN_BODY_LEN: usize = 100;

/// Jina Reader base URL for content extraction.
...
impl WebFetchTool {
    ...
    /// Whether the standard fetch result should trigger a fallback (Jina Reader or Firecrawl).
    ...
    /// Whether Jina Reader fallback should be attempted (always available, no API key required).
    ...
    /// Fetch clean content via Jina Reader (free, no API key required).
    /// Jina Reader extracts main content from any URL and returns it as clean markdown.
...
impl Tool for WebFetchTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Fallback chain: Jina Reader (free, no key) -> Firecrawl (requires API key)
...
mod tests {
    ...
    async fn firecrawl_missing_api_key_returns_error() {
        ...
        crate::util::remove_env_var("FIRECRAWL_TEST_MISSING_KEY");
    ...
    async fn execute_double_failure_returns_original_result() {
        ...
        crate::util::remove_env_var("FIRECRAWL_DOUBLE_FAIL_KEY");
    ...
    async fn execute_falls_back_to_firecrawl_on_short_body() {
        ...
        crate::util::set_env_var("FIRECRAWL_E2E_TEST_KEY", "test-key-12345");
        ...
        // Clean up env var
        crate::util::remove_env_var("FIRECRAWL_E2E_TEST_KEY");
    }
