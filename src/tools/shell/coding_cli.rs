...
use crate::config::ShellCodingCliConfig;
...
pub struct ShellCodingCliTool {
    security: Arc<SecurityPolicy>,
    config: ShellCodingCliConfig,
}

impl ShellCodingCliTool {
    pub fn new(security: Arc<SecurityPolicy>, config: ShellCodingCliConfig) -> Self {
        Self { security, config }
    }
}

#[async_trait]
impl Tool for ShellCodingCliTool {
    fn name(&self) -> &str {
        "shell_coding_cli"
    }

    fn description(&self) -> &str {
        "Delegate a coding task to an external coding CLI with shell execution support (binary 'gemini -p'). Supports file editing and shell execution. Use for coding work that benefits from a shell-capable external agent loop."
    }
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                "prompt": {
                    "type": "string",
                    "description": "The coding task to delegate to the shell coding CLI"
                },
                "working_directory": {
                    "type": "string",
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if let Err(error) = self
            .security
            .enforce_tool_operation(ToolOperation::Act, "shell_coding_cli")
        {
            return Ok(ToolResult {
                success: false,
                output: String::new(),
        ...
        match result {
            ...
            Ok(Err(e)) => {
                ...
                {
                    "Shell coding CLI ('gemini') not found in PATH. Configure the binary via PATH or update [shell_coding_cli] in config.".into()
                } else {
                    format!("Failed to execute shell coding CLI: {e}")
                };
                Ok(ToolResult {
                    success: false,
                    output: String::new(),
            ...
            Err(_) => {
                ...
                Ok(ToolResult {
                    ...
                    output: String::new(),
                    error: Some(format!(
                        "Shell coding CLI timed out after {}s and was killed",
