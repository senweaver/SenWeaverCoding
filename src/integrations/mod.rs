...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `sen onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: sen onboard --channels-only");
            println!("    4. Start: sen channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: sen onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    SenWeaverCoding can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.senweavercoding/workspace/cron/");
            println!("    Run: sen cron list");
        }
        "Weather" => {
            println!("  Built-in:");
        ...
        "Webhooks" => {
            ...
            println!("    Run: sen gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/senweaver/SenWeaverCoding");
            }
        }
    }
