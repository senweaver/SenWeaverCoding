...
/// Skills live in `~/.senweavercoding/workspace/skills/<name>/SKILL.md`
...
/// Discover all skills that pass audit (workspace `skills/` plus optional open-skills), without applying
/// `[skills].disabled_skills`. Used by the dashboard API to list skills while still showing disabled entries.
...
/// Remove skills whose `name` appears in `disabled` (after trim; empty entries ignored).
    ...
    impl Drop for EnvVarGuard {
        fn drop(&mut self) {
            if let Some(value) = &self.original {
                crate::util::set_env_var(self.key, value);
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.senweavercoding");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.senweavercoding/skills"));
    }
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("SEN_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("SEN_OPEN_SKILLS_DIR");
