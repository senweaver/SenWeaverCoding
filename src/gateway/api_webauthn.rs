...
pub struct WebAuthnState {
    ...
    /// Pending registration states keyed by challenge (with creation timestamp).
    pub pending_registrations: Mutex<HashMap<String, (RegistrationState, std::time::Instant)>>,
    /// Pending authentication states keyed by challenge (with creation timestamp).
    pub pending_authentications: Mutex<HashMap<String, (AuthenticationState, std::time::Instant)>>,
}
