...
pub async fn handle_api_session_messages(
    ...
    let pending_rewind = backend.latest_rewind_stash_for_session(&session_key).map(|stash| {
        ...
        let workspace = resolve_session_workspace(&state, &session_key);
...
pub async fn handle_api_session_rewind(
    ...
    let live: Vec<(usize, &crate::channels::session_backend::LoadedMessage)> = loaded
        .iter()
        .enumerate()
    ...
    let batches_after = backend.edit_batches_after(&session_key, target_user_idx);
    ...
    while let Some(prev) = backend.latest_rewind_stash_for_session(&session_key) {
        if let Ok(entries) =
        ...
        let _ = backend.take_rewind_stash(&prev.rewind_id);
    }
    ...
    let stash_entries: Vec<RewindStashEntry> = entries_by_path
        ...
        .collect();
    let stash_json = serde_json::to_string(&stash_entries).unwrap_or_else(|_| "[]".to_string());
    if let Err(e) =
        backend.save_rewind_stash(&rewind_id, &session_key, target_user_idx, &stash_json)
    {
    ...
    let tombstoned = match backend.tombstone_from(&session_key, first_db_id) {
        Ok(n) => n,
        Err(e) => {
...
pub async fn handle_api_session_rewind_restore(
    ...
    let Some(stash) = backend.take_rewind_stash(&body.rewind_id) else {
        return (
            StatusCode::NOT_FOUND,
    ...
    let cleared = match backend.clear_tombstones(&session_key) {
        Ok(n) => n,
        Err(e) => {
...
pub async fn handle_api_session_rewind_commit(
    ...
    let stash = backend.take_rewind_stash(&body.rewind_id);
    let user_message_index = stash.as_ref().map(|s| s.user_message_index).unwrap_or(0);

    let purged = match backend.purge_tombstoned(&session_key) {
        Ok(n) => n,
        Err(e) => {
    ...
    if stash.is_some() {
        let _ = backend.drop_edit_batches_after(&session_key, user_message_index);
    }
...
pub async fn handle_api_session_revert_batches(
    ...
    for batch_id in body.edit_batch_ids.iter().rev() {
        match history.revert_batch(batch_id) {
            Ok(paths) => {
                if paths.is_empty() {
