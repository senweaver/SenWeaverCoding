...
pub mod nodes;
pub mod sse;
// Web dashboard removed — CLI code editor does not need it.
...
/// Read gateway request timeout from `SEN_GATEWAY_TIMEOUT_SECS` env var
...
pub struct AppState {
    ...
    /// RBAC engine for user/role/permission management
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    // Global token optimizer + guardrails (idempotent if Agent already initialized them)
    crate::agent::token_optimizer::ensure_global_optimizer(
    ...
    // Build A2A (Agent-to-Agent) protocol router
    ...
    let inner = inner
        //  -  -  Suggestions / Guardrails / Tool Groups API  -  - 
        ...
        .route("/api/agents", get(api::handle_api_agents_list))
        .route("/api/agents/status", get(api::handle_api_agents_status))
        .route("/api/tasks", get(api::handle_api_tasks_status))
        ...
        //  -  -  Hardware context management API  -  -
...
mod tests {
    ...
    fn gateway_timeout_falls_back_to_default() {
        ...
        crate::util::remove_env_var("SEN_GATEWAY_TIMEOUT_SECS");
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("sen_heartbeat_ticks_total 1"));
    }
