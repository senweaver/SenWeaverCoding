...
pub async fn submit_pairing_enhanced(
    ...
    // X-Forwarded-For is trivially spoofable; do NOT use it for
    // security-sensitive rate limiting unless behind a trusted reverse proxy.
    // Prefer x-real-ip (typically set by nginx) and fall back to "unknown".
