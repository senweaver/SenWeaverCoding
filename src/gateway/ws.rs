...
async fn handle_socket(
    ...
    let mut agent = match crate::agent::Agent::from_config(&config, None).await {
        Ok(a) => a,
        Err(e) => {
...
async fn process_chat_message(
    ...
    match result {
        Ok(response) => {
            ...
            // Auto-title: generate a session title after the first exchange
            let auto_title_config = state.config.lock().auto_title.clone();
            if auto_title_config.enabled {
                if let Some(ref backend) = state.session_backend {
                    let existing_name = backend.get_session_name(session_key).ok().flatten();
                    if existing_name.is_none() {
                        ...
                        .await
                        {
                            let _ = backend.set_session_name(session_key, &title);
            ...
            // Publish message events to event bus
...
mod tests {
    ...
    fn extract_ws_token_from_subprotocol() {
        ...
        headers.insert(
            ...
            "sen.v1, bearer.zc_sub456".parse().unwrap(),
        );
    ...
    fn extract_ws_token_subprotocol_with_multiple_entries() {
        ...
        headers.insert(
            "sec-websocket-protocol",
            "sen.v1, bearer.zc_tok, other".parse().unwrap(),
        );
