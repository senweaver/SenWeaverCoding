...
/// Core verification rules — injected in **all** modes.
///
/// Prevents the agent from falsely claiming success without evidence.
...
/// TDD rules — injected in **TDD** mode.
...
/// Debug rules — injected in **Debug** mode.
...
/// Planning rules — injected in **Plan** and **Spec** modes.
...
/// Code review rules — available for `/review` enhancement.
...
/// Agent mode rules — injected in **Agent** mode.
///
/// Unified autonomous agent: combines fully autonomous execution with structured
/// task decomposition, orchestration, and quality gates.
...
/// Architect mode rules — injected in **Architect** mode.
///
/// High-level design and code review with implementation delegation.
...
/// Pair-programming rules — injected in **Pair** mode.
///
/// Interactive collaborative workflow with explicit checkpoints.
...
/// Context Engineering rules — injected in **ContextEng** mode.
///
/// Four-phase explore-first protocol for large-codebase precision work.
...
/// MVAI (Model-View-Agent-Interface) rules — injected in **Mvai** mode.
///
/// Structured contracts, observable agent layer, deterministic testing.
...
/// Harness Engineering-Grade Workflow rules — injected in **Harness** mode.
///
/// Combines the best of OpenSpec, Superpowers, GSD, OMC, ECC, and Trellis into
/// a layered engineering workflow: Spec → Skill Orchestration → Session Management → Verification.
...
mod tests {
    ...
    fn all_rules_non_empty() {
        assert!(!verification_rules().is_empty());
        assert!(!tdd_rules().is_empty());
        assert!(!debug_rules().is_empty());
        assert!(!planning_rules().is_empty());
        assert!(!review_rules().is_empty());
        assert!(!agent_rules().is_empty());
        assert!(!architect_rules().is_empty());
        assert!(!pair_rules().is_empty());
        assert!(!context_eng_rules().is_empty());
        assert!(!mvai_rules().is_empty());
        assert!(!harness_rules().is_empty());
    }

    #[test]
    fn verification_contains_evidence_keyword() {
        assert!(verification_rules().contains("Evidence"));
    }

    #[test]
    fn tdd_contains_red_green_refactor() {
        let rules = tdd_rules();
        assert!(rules.contains("Red"));
        assert!(rules.contains("Green"));
        assert!(rules.contains("Refactor"));
    }

    #[test]
    fn debug_contains_four_stages() {
        let rules = debug_rules();
        assert!(rules.contains("Reproduce"));
        assert!(rules.contains("Hypothesize"));
        assert!(rules.contains("Isolate"));
        assert!(rules.contains("Fix & Verify"));
    }

    #[test]
    fn agent_contains_decompose_and_self_correct() {
        let rules = agent_rules();
        assert!(rules.contains("Self-correct"));
        assert!(rules.contains("Decompose"));
        assert!(rules.contains("Synthesize"));
        assert!(rules.contains("Quality Gates"));
    }

    #[test]
    fn architect_contains_design() {
        assert!(architect_rules().contains("Architect"));
        assert!(architect_rules().contains("Design"));
    }

    #[test]
    fn pair_contains_collaborative() {
        assert!(pair_rules().contains("Pair"));
        assert!(pair_rules().contains("collaborat"));
    }

    #[test]
    fn context_eng_has_four_phases() {
        let rules = context_eng_rules();
        assert!(rules.contains("Phase 1: Explore"));
        assert!(rules.contains("Phase 2: Map"));
        assert!(rules.contains("Phase 3: Strike"));
        assert!(rules.contains("Phase 4: Consolidate"));
        assert!(rules.contains("Tool Discipline"));
        assert!(rules.contains("content_search"));
    }

    #[test]
    fn mvai_contains_interface() {
        assert!(mvai_rules().contains("MVAI"));
        assert!(mvai_rules().contains("Interface"));
    }

    #[test]
    fn harness_contains_all_six_layers() {
        let rules = harness_rules();
        assert!(rules.contains("Layer 1: Spec Layer"), "missing Spec Layer");
        assert!(rules.contains("Layer 2: Skill Orchestration Layer"), "missing Skill Orchestration Layer");
        assert!(rules.contains("Layer 3: Session Management Layer"), "missing Session Management Layer");
        assert!(rules.contains("Layer 4: Multi-Agent Orchestration Layer"), "missing Multi-Agent Orchestration Layer");
        assert!(rules.contains("Layer 5: Capability Enhancement Layer"), "missing Capability Enhancement Layer");
        assert!(rules.contains("Layer 6: Structure and Project Memory Layer"), "missing Structure and Project Memory Layer");
    }

    #[test]
    fn harness_contains_opencode_references() {
        let rules = harness_rules();
        assert!(rules.contains("OpenSpec") || rules.contains("agree before you build"));
        assert!(rules.contains("Superpowers") || rules.contains("Engineering Discipline"));
        assert!(rules.contains("GSD") || rules.contains("Context Rot"));
        assert!(rules.contains("OMC") || rules.contains("Multi-Agent"));
        assert!(rules.contains("ECC") || rules.contains("Security"));
        assert!(rules.contains("Trellis") || rules.contains("workspace"));
    }

    #[test]
    fn harness_verification_sequence() {
        let rules = harness_rules();
        assert!(rules.contains("cargo check") || rules.contains("check"));
        assert!(rules.contains("cargo test") || rules.contains("test"));
        assert!(rules.contains("verification") || rules.contains("Verify"));
    }
}
