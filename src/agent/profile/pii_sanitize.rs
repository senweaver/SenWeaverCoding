...
pub(crate) fn pii_sanitize_text_outside_image_markers(
    input: &str,
) -> (String, crate::services::governance::pii_sanitizer::SanitizationReport) {
    let mut combined =
        crate::services::governance::pii_sanitizer::SanitizationReport::default();
    if input.is_empty() {
    ...
    while let Some(rel) = input[cursor..].find(marker) {
        ...
        if !prefix.is_empty() {
            let (clean, report) = crate::services::governance::pii_sanitizer::sanitize_text(prefix);
    ...
    if cursor < input.len() {
        let (clean, report) = crate::services::governance::pii_sanitizer::sanitize_text(&input[cursor..]);
        combined.merge(&report);
        output.push_str(&clean);
    }
...
fn pii_sanitize_json_value_image_aware(
    ...
    report: &mut crate::services::governance::pii_sanitizer::SanitizationReport,
) -> serde_json::Value {
    match value {
        serde_json::Value::String(s) => {
            if s.contains("[IMAGE:") {
                ...
                let (clean, sub) = crate::services::governance::pii_sanitizer::sanitize_text(s);
                report.merge(&sub);
                serde_json::Value::String(clean)
            }
        }
...
pub(crate) fn scrub_credentials(input: &str) -> String {
    let after_vault =
        crate::services::governance::credential_vault::redact_for_audit_optional(input);
    SENSITIVE_KV_REGEX
