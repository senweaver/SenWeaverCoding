...
pub struct Agent {
    ...
    /// Personalization and learning-related prompt injections (from top-level `Config`).
    ...
    /// Optional RBAC for [`turn`] / [`turn_streamed`] tool path (e.g. gateway WebSocket).
...
impl Agent {
    ...
    /// Execute a single tool by name with the given JSON arguments.
    /// Returns the tool execution result. Used by the RPC server for direct tool invocation.
    ...
    /// Attach RBAC for the streaming/non-loop tool path (`turn` / `turn_streamed`).
    ...
    /// Register [`crate::tools::NodeTool`] entries for every capability on connected nodes.
    /// Used by the gateway WebSocket chat path when `[nodes] enabled = true`.
    ...
    pub async fn from_config(
        ...
        // Apply denied_tools filter if provided.
    ...
    fn build_system_prompt(&self) -> Result<String> {
        ...
        // Inject user profile context if available (uses configured limits).
    ...
    async fn execute_tool_call(&self, call: &ParsedToolCall) -> ToolExecutionResult {
        ...
        if let Err(reason) = crate::guardrails::check_tool_guardrails(&call.name) {
            return ToolExecutionResult {
                name: call.name.clone(),
        ...
        let (output, success) =
            if let Some(tool) = self.tools.iter().find(|t| t.name() == call.name) {
                run_tool(tool.as_ref(), call, &self.observer).await
        ...
        crate::agent::runtime_hooks::publish_tool_event(&call.name, success, wall_ms);
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        let enriched = Self::build_user_envelope(user_message, &context);
...
mod tests {
    ...
    async fn from_config_passes_extra_headers_to_custom_provider() {
        ...
        config.extra_headers.insert(
            ...
            "sen-web-test/1.0".to_string(),
        );
        config
            .extra_headers
            .insert("X-Title".to_string(), "sen-web".to_string());
