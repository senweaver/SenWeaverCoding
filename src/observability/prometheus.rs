...
impl PrometheusObserver {
    pub fn new() -> Self {
        ...
        let mttr =
            prometheus::Gauge::new("sen_mttr_seconds", "Mean time to recovery in seconds")
                .expect("valid metric");
...
mod tests {
    ...
    fn encode_produces_prometheus_text_format() {
        ...
        assert!(output.contains("sen_agent_starts_total"));
        assert!(output.contains("sen_tool_calls_total"));
        assert!(output.contains("sen_heartbeat_ticks_total"));
        assert!(output.contains("sen_request_latency_seconds"));
    }
    ...
    fn counters_increment_correctly() {
        ...
        let output = obs.encode();
        assert!(output.contains("sen_heartbeat_ticks_total 3"));
    }
    ...
    fn tool_calls_track_success_and_failure_separately() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"sen_tool_calls_total{success="true",tool="shell"} 2"#));
        assert!(output.contains(r#"sen_tool_calls_total{success="false",tool="shell"} 1"#));
    }
    ...
    fn errors_track_by_component() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"sen_errors_total{component="provider"} 2"#));
        assert!(output.contains(r#"sen_errors_total{component="channels"} 1"#));
    }
    ...
    fn gauge_reflects_latest_value() {
        ...
        let output = obs.encode();
        assert!(output.contains("sen_tokens_used_last 200"));
    }
    ...
    fn llm_response_tracks_request_count_and_tokens() {
        ...
        assert!(output.contains(
            r#"sen_llm_requests_total{model="claude-sonnet",provider="openrouter",success="true"} 2"#
        ));
        assert!(output.contains(
            r#"sen_tokens_input_total{model="claude-sonnet",provider="openrouter"} 300"#
        ));
        assert!(output.contains(
            r#"sen_tokens_output_total{model="claude-sonnet",provider="openrouter"} 130"#
        ));
    }
    ...
    fn hand_events_track_runs_and_duration() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"sen_hand_runs_total{hand="review",success="true"} 2"#));
        assert!(output.contains(r#"sen_hand_runs_total{hand="review",success="false"} 1"#));
        assert!(output.contains(r#"sen_hand_findings_total{hand="review"} 4"#));
        assert!(output.contains("sen_hand_duration_seconds"));
    }
    ...
    fn hand_metrics_record_duration_and_findings() {
        ...
        let output = obs.encode();
        assert!(output.contains("sen_hand_duration_seconds"));
        assert!(output.contains(r#"sen_hand_findings_total{hand="scan"} 5"#));
        assert!(output.contains(r#"sen_hand_runs_total{hand="scan",success="true"} 1"#));
        assert!(output.contains(r#"sen_hand_runs_total{hand="scan",success="false"} 1"#));
    }
    ...
    fn llm_response_without_tokens_increments_request_only() {
        ...
        let output = obs.encode();
        assert!(output.contains(
            r#"sen_llm_requests_total{model="llama3",provider="ollama",success="false"} 1"#
        ));
        // Token counters should not appear (no data recorded)
        assert!(!output.contains("sen_tokens_input_total{"));
        assert!(!output.contains("sen_tokens_output_total{"));
    }
    ...
    fn dora_deployment_events_track_counters() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"sen_deployments_total{status="success"} 2"#));
        assert!(output.contains(r#"sen_deployments_total{status="failure"} 1"#));
    }
    ...
    fn dora_failure_rate_gauge_updates() {
        ...
        assert!(output.contains("sen_deployment_failure_rate 0.5"));
    }
    ...
    fn dora_lead_time_and_recovery_metrics() {
        ...
        let output = obs.encode();
        assert!(output.contains("sen_deployment_lead_time_seconds"));
        assert!(output.contains("sen_recovery_time_seconds"));
        assert!(output.contains("sen_mttr_seconds 600"));
    }
