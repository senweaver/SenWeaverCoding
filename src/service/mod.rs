...
/// Returns whether the SenWeaverCoding daemon service is currently running.
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "sen.service"]));
        }
        InitSystem::Openrc => {
...
fn logs_linux(config: &Config, init_system: InitSystem, lines: usize, follow: bool) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            // OpenRC logs go to /var/log/sen/error.log (as configured in the init script)
    ...
    match init_system {
        ...
        InitSystem::Openrc => {
            ...
            if init_script.exists() {
                ...
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove sen from OpenRC default runlevel: {err}"
                    );
                }
...
/// corresponding `var/sen` directory.
///
...
fn detect_homebrew_var_dir(exe: &Path) -> Option<PathBuf> {
    ...
    // Symlinked binary: <prefix>/bin/sen
    // Cellar binary:    <prefix>/Cellar/sen/<version>/bin/sen
    let prefix = if path_str.contains("/Cellar/") {
        // Walk up from .../Cellar/sen/<ver>/bin/sen to the prefix
        ...
        // <prefix>/bin/sen → check if <prefix>/Cellar exists (Homebrew marker)
...
fn install_macos(config: &Config) -> Result<()> {
    ...
    // data so that `brew services start sen` works out of the box.
    ...
    // When running under Homebrew, inject SEN_CONFIG_DIR and
...
/// Check if the sen user exists and has expected properties.
    ...
    match output {
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if home != "/var/lib/sen" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: sen user has home directory '{}' (expected /var/lib/sen or /nonexistent)",
                        home
                    );
                }
...
/// Change ownership of a path to sen:sen
#[cfg(unix)]
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/sen.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "SenWeaverCoding Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/sen");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/sen"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"sen\""));
        assert!(script.contains("description=\"SenWeaverCoding daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/sen\""));
        assert!(script.contains("command_args=\"--config-dir /etc/sen daemon\""));
        assert!(!script.contains("env SEN_CONFIG_DIR"));
        assert!(!script.contains("env SEN_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"sen:sen\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/sen/access.log\""));
        assert!(script.contains("error_log=\"/var/log/sen/error.log\""));
    ...
    fn generate_openrc_script_sets_home_for_browser() {
        ...
        assert!(
            script.contains("export HOME=\"/var/lib/sen\""),
    ...
    fn generate_openrc_script_creates_home_directory() {
        ...
        assert!(
            script.contains("checkpath --directory --owner sen:sen"),
            "start_pre must ensure /var/lib/sen exists with correct ownership"
        );
    }
    ...
    fn systemd_unit_contains_home_and_pass_environment() {
        let unit = "[Unit]\n\
             ...
             [Service]\n\
             Type=simple\n\
             ExecStart=/usr/local/bin/sen daemon\n\
             Restart=always\n\
             RestartSec=3\n\
             # Ensure HOME is set so headless browsers can create profile/cache dirs.\n\
             Environment=HOME=%h\n\
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/sen"), true);
    ...
    fn detect_homebrew_var_dir_from_cellar_path() {
        let exe = PathBuf::from("/opt/homebrew/Cellar/sen/1.2.3/bin/sen");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/opt/homebrew/var/sen")));
    }

    #[test]
    fn detect_homebrew_var_dir_intel_cellar_path() {
        let exe = PathBuf::from("/usr/local/Cellar/sen/1.0.0/bin/sen");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/usr/local/var/sen")));
    }

    #[test]
    fn detect_homebrew_var_dir_non_homebrew_path() {
        let exe = PathBuf::from("/home/user/.cargo/bin/sen");
        let var_dir = detect_homebrew_var_dir(&exe);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/sen/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/sen/workspace'".to_string(),
                "sen".to_string()
            ]
        );
    }
    ...
    fn tail_file_errors_on_missing_file() {
        let missing = Path::new("/tmp/sen-test-nonexistent-log-file.log");
