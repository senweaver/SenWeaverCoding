...
struct Inner {
    ...
    file_mtimes: HashMap<(String, PathBuf), SystemTime>,
}
...
impl WorkspaceResourceManager {
    ...
    pub async fn acquire(
        ...
        let announce_workspace = workspace_key.to_string();
        ...
        match wait_result {
            Ok(Ok(())) => {
                self.emit_wait_resolved(workspace_key, &kind, session_id);
                Ok(ResourceGuard {
                    manager: Arc::clone(self),
                    workspace_key: workspace_key.to_string(),
                    kind,
            ...
            Ok(Err(_)) => {
                self.cancel_waiter(workspace_key, &kind, session_id);
    ...
    pub async fn acquire_many(
        ...
        kinds.sort_by(|a, b| a.sort_key().cmp(&b.sort_key()));
        kinds.dedup();
    ...
    pub fn record_read(&self, workspace_key: &str, session_id: &str, path: &Path) {
        ...
        let mut inner = self.inner.lock();
        if let Some(t) = mtime {
            inner.read_snapshots.insert(key, t);
        } else {
            inner.read_snapshots.insert(key, SystemTime::UNIX_EPOCH);
        }
    }

    pub fn record_write(&self, workspace_key: &str, session_id: &str, path: &Path) {
        let mtime = fs_mtime(path).unwrap_or_else(SystemTime::now);
        let key = (workspace_key.to_string(), path.to_path_buf());
        let mut inner = self.inner.lock();
        inner.file_mtimes.insert(key.clone(), mtime);
        let snap_key = (
            workspace_key.to_string(),
            session_id.to_string(),
            path.to_path_buf(),
