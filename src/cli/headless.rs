...
/// Why the headless session ended.
...
/// Configuration for the headless driver.
#[derive(Debug, Clone)]
...
/// Run the agent in headless/SDK mode.
///
/// This is the main entry point for non-interactive sessions. It:
/// 1. Sends the initial prompt to the model
/// 2. Processes model responses (text, tool_use)
/// 3. For each tool_use, emits a ControlRequest via io.request_permission()
/// 4. Executes approved tools
/// 5. Feeds tool results back to the model
/// 6. Repeats until end_turn, max_turns, or error
/// 7. Returns a SessionResult
pub async fn run_headless(config: HeadlessConfig, io: &mut StructuredIO) -> Result<SessionResult> {
    ...
    // Emit the initial prompt as an assistant message for SDK consumers
    if config.output_format == OutputFormat::StreamJson {
        io.emit_system(&format!("Session {} started", config.session_id));
    }

    // Load the runtime config once for the entire session.
    ...
    // Configure permission mode for headless/SDK
    ...
    // Initial turn: run the agent with the headless config's initial prompt.
    ...
    loop {
        ...
        // Wait for a follow-up message or abort signal from the SDK host.
        match io.recv().await {
...
/// Handle a permission request for a tool use via the SDK control protocol.
///
/// Sends a `can_use_tool` control_request and waits for the SDK host's response.
...
/// Check if a tool is in the allow/deny lists.
pub fn is_tool_allowed(tool_name: &str, allowed: &[String], denied: &[String]) -> Option<bool> {
    ...
    None // No explicit decision; defer to permission system
}

/// Join multiple prompt values into a single string (for multi-part prompts).
...
mod tests {
    ...
    #[test]
    fn exit_reason_serializes() {
        let json = serde_json::to_string(&ExitReason::MaxTurns).unwrap();
    ...
    #[test]
    fn tool_allowed_empty_lists() {
        assert_eq!(is_tool_allowed("shell", &[], &[]), None);
    }

    #[test]
    fn tool_denied() {
        let denied = vec!["shell".to_string()];
        assert_eq!(is_tool_allowed("shell", &[], &denied), Some(false));
        assert_eq!(is_tool_allowed("file_read", &[], &denied), None);
    }

    #[test]
    fn tool_allowed_with_allowlist() {
        let allowed = vec!["file_read".to_string(), "glob_search".to_string()];
        assert_eq!(is_tool_allowed("file_read", &allowed, &[]), None);
        assert_eq!(is_tool_allowed("shell", &allowed, &[]), Some(false));
    }

    #[test]
    fn wildcard_deny() {
        let denied = vec!["*".to_string()];
        assert_eq!(is_tool_allowed("anything", &[], &denied), Some(false));
    }

    #[test]
    fn join_prompts() {
        let vals = vec!["first".into(), "second".into()];
        assert_eq!(join_prompt_values(&vals), "first\n\nsecond");
    }
