...
//! Agent listing handler — shows configured delegate agents.

use anyhow::Result;
use std::path::Path;

/// List configured agents from the workspace configuration.
pub async fn list_agents(workspace: &Path) -> Result<Vec<AgentInfo>> {
    ...
    // Parse agent configs from the TOML configuration
...
/// Information about a configured agent.
#[derive(Debug, Clone)]
...
/// Where the agent configuration comes from.
