...
impl BatchUploader {
    /// Create a new batch uploader and start the background flush loop.
    ...
    /// Get the total events enqueued.
    pub fn events_sent(&self) -> u64 {
        self.events_sent.load(Ordering::Relaxed)
    }

    /// Get the total events dropped due to failures.
    pub fn events_dropped(&self) -> u64 {
        self.events_dropped.load(Ordering::Relaxed)
    }

    /// Upload configuration (URL, batch limits, backoff).
...
mod tests {
    ...
    #[test]
    fn default_config() {
        let cfg = BatchUploaderConfig::default();
