...
mod tests {
    ...
    fn request_serializes_correctly() {
        let req = ApiChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: MessageContent::Text("You are SenWeaverCoding".to_string()),
                },
                Message {
                    role: "user".to_string(),
    ...
    fn user_agent_constructor_keeps_native_tool_calling_enabled() {
        ...
        assert!(caps.native_tool_calling);
        assert!(!caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("sen-test/1.0"));
    }
    ...
    fn user_agent_and_vision_constructor_preserves_capability_flags() {
        ...
        assert!(caps.native_tool_calling);
        assert!(caps.vision);
        assert_eq!(p.user_agent.as_deref(), Some("sen-test/vision"));
    }
    ...
    fn with_extra_headers_sets_headers() {
        ...
        headers.insert("X-Title".to_string(), "sen".to_string());
        headers.insert(
        ...
        assert_eq!(p.extra_headers.get("X-Title").unwrap(), "sen");
        assert_eq!(
