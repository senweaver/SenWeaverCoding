...
pub struct ProviderRuntimeOptions {
    ...
    /// These are merged from the config file and `SEN_EXTRA_HEADERS` env var.
...
/// 3. Generic fallback variables (`SEN_API_KEY`, `API_KEY`)
///
...
/// Return the list of all known providers for display in `sen providers list`.
///
...
mod tests {
    ...
    impl EnvGuard {
        fn set(key: &'static str, value: Option<&str>) -> Self {
            ...
            match value {
                Some(next) => crate::util::set_env_var(key, next),
                None => crate::util::remove_env_var(key),
            }
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            if let Some(original) = self.original.as_deref() {
                crate::util::set_env_var(self.key, original);
            } else {
                crate::util::remove_env_var(self.key);
            }
        }
    }
    ...
    fn resolve_qwen_oauth_context_prefers_explicit_override() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/sen-qwen-oauth-home-{}", std::process::id());
    ...
    fn resolve_qwen_oauth_context_uses_env_token_and_resource_url() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/sen-qwen-oauth-home-{}-env", std::process::id());
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, Some("oauth-token"));
    ...
    fn resolve_qwen_oauth_context_reads_cached_credentials_file() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/sen-qwen-oauth-home-{}-file", std::process::id());
    ...
    fn resolve_qwen_oauth_context_placeholder_does_not_use_dashscope_fallback() {
        ...
        let fake_home = format!(
            "/tmp/sen-qwen-oauth-home-{}-placeholder",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
    ...
    fn resolve_provider_credential_opencode_go_env() {
        ...
        let _sen_guard = EnvGuard::set("SEN_API_KEY", None);
    ...
    fn provider_runtime_options_extra_headers_passed_through() {
        ...
        extra_headers.insert("X-Title".to_string(), "sen".to_string());
        let options = ProviderRuntimeOptions {
            extra_headers,
            ..ProviderRuntimeOptions::default()
        };
        assert_eq!(options.extra_headers.len(), 1);
        assert_eq!(options.extra_headers.get("X-Title").unwrap(), "sen");
    }

    #[test]
    fn env_provider_url_overrides_api_url() {
        crate::util::set_env_var("SEN_PROVIDER_URL", "http://env-ollama:11434");
        ...
        crate::util::remove_env_var("SEN_PROVIDER_URL");
    }
}
