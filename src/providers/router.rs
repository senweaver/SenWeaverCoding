...
impl RouterProvider {
    ...
    /// Attach model pricing data for cost-optimized routing.
    ...
    /// Resolve with automatic cost-hint detection.
    ///
    /// If the model starts with `hint:cost` or `hint:cheap` and pricing data
    /// is available, delegates to [`resolve_cost_optimized`]. Otherwise falls
    /// through to the standard [`resolve`] path.
