...
mod tests {
    ...
    fn new_uses_env_override() {
        ...
        crate::util::set_env_var(GEMINI_CLI_PATH_ENV, "/usr/local/bin/gemini");
        let provider = GeminiCliProvider::new();
        assert_eq!(provider.binary_path, PathBuf::from("/usr/local/bin/gemini"));
        match orig {
            Some(v) => crate::util::set_env_var(GEMINI_CLI_PATH_ENV, v),
            None => crate::util::remove_env_var(GEMINI_CLI_PATH_ENV),
        }
    }
    ...
    fn new_defaults_to_gemini() {
        let _guard = env_lock();
        let orig = std::env::var(GEMINI_CLI_PATH_ENV).ok();
        crate::util::remove_env_var(GEMINI_CLI_PATH_ENV);
        let provider = GeminiCliProvider::new();
        assert_eq!(provider.binary_path, PathBuf::from("gemini"));
        if let Some(v) = orig {
            crate::util::set_env_var(GEMINI_CLI_PATH_ENV, v);
        }
    }
    ...
    fn new_ignores_blank_env_override() {
        let _guard = env_lock();
        let orig = std::env::var(GEMINI_CLI_PATH_ENV).ok();
        crate::util::set_env_var(GEMINI_CLI_PATH_ENV, "   ");
        let provider = GeminiCliProvider::new();
        assert_eq!(provider.binary_path, PathBuf::from("gemini"));
        match orig {
            Some(v) => crate::util::set_env_var(GEMINI_CLI_PATH_ENV, v),
