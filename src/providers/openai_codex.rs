...
impl OpenAiCodexProvider {
    pub fn new(
        ...
        let state_dir = options
            .sen_dir
            .clone()
            .unwrap_or_else(default_sen_dir);
...
mod tests {
    ...
    fn resolve_reasoning_effort_prefers_configured_override() {
        let _lock = env_lock();
        let _guard = EnvGuard::set("SEN_CODEX_REASONING_EFFORT", Some("low"));
        assert_eq!(
    ...
    fn resolve_reasoning_effort_uses_legacy_env_when_unconfigured() {
        let _lock = env_lock();
        let _guard = EnvGuard::set("SEN_CODEX_REASONING_EFFORT", Some("minimal"));
        assert_eq!(
