...
mod tests {
    ...
    fn new_uses_env_override() {
        ...
        crate::util::set_env_var(CLAUDE_CODE_PATH_ENV, "/usr/local/bin/claude");
        let provider = ClaudeCodeProvider::new();
        assert_eq!(provider.binary_path, PathBuf::from("/usr/local/bin/claude"));
        match orig {
            Some(v) => crate::util::set_env_var(CLAUDE_CODE_PATH_ENV, v),
            None => crate::util::remove_env_var(CLAUDE_CODE_PATH_ENV),
        }
    }
    ...
    fn new_defaults_to_claude() {
        let _guard = env_lock();
        let orig = std::env::var(CLAUDE_CODE_PATH_ENV).ok();
        crate::util::remove_env_var(CLAUDE_CODE_PATH_ENV);
        let provider = ClaudeCodeProvider::new();
        assert_eq!(provider.binary_path, PathBuf::from("claude"));
        if let Some(v) = orig {
            crate::util::set_env_var(CLAUDE_CODE_PATH_ENV, v);
        }
    }
    ...
    fn new_ignores_blank_env_override() {
        let _guard = env_lock();
        let orig = std::env::var(CLAUDE_CODE_PATH_ENV).ok();
        crate::util::set_env_var(CLAUDE_CODE_PATH_ENV, "   ");
        let provider = ClaudeCodeProvider::new();
        assert_eq!(provider.binary_path, PathBuf::from("claude"));
        match orig {
            Some(v) => crate::util::set_env_var(CLAUDE_CODE_PATH_ENV, v),
