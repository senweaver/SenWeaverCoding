...
impl LocalCliProvider {
    ...
    fn validate_temperature(temperature: f64) -> anyhow::Result<f64> {
        ...
        let clamped = *LOCAL_CLI_SUPPORTED_TEMPERATURES
            .iter()
            .min_by(|a, b| {
                (temperature - **a)
                    .abs()
            ...
            .ok_or_else(|| {
                anyhow::anyhow!("Local CLI provider has no supported temperatures configured")
            })?;
        tracing::debug!(
            requested = temperature,
            clamped = clamped,
            "Clamped unsupported temperature to nearest local CLI value"
        );
        Ok(clamped)
    }
